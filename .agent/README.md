...
- **Identity:** [`IDENTITY.md`](IDENTITY.md) is the canonical identity document — Castr is a schema compiler with strict-by-default object semantics, honest preservation of explicit source-truth `additionalProperties`, and no invented openness
...
- **Explicit `additionalProperties` truth:** the semantic implementation landed on Thursday, 16 April 2026, and the broad doc / staged-history consolidation closed on Friday, 17 April 2026. No successor atomic slice has been promoted yet, so [`.agent/plans/active/explicit-additional-properties-support.md`](plans/active/explicit-additional-properties-support.md) remains the honest regression-first / successor-selection entrypoint
- **Next-step truth:** do not reopen AP4, Phase B, Phase C, Phase D, or Phase E unless a fresh regression is reproduced
- **Immediate next slice:** if a user reports a fresh gate or runtime issue, reproduce it first; otherwise use [`.agent/plans/active/explicit-additional-properties-support.md`](plans/active/explicit-additional-properties-support.md) only as the honest regression-first / successor-selection entrypoint until a real successor atomic slice is promoted
