...
and zero unpushed work. The two exceptions once named here (#81 open until C09; #23's
branch retained after its PR closes, per the 26 August ruling) are retracted: the
owner ruled on 21 September that "work that we don't want needn't be on a branch",
so #81 closes at the unmerged-work plan's W2-8 and #23 at its W2-1, branches
deleted. Nothing on a branch is safe. Zero is reached by proving value and preserving
it on main, and otherwise by deleting with the reason recorded on main; it is never
reached by merging containers or by
...
2. Owner rulings, 20 and 21 September: every piece of unmerged work reaches main or
   is deleted, and nothing is routed to any bucket of later, whatever it is called.
   The executable plan
   [unmerged-work-to-main-or-deleted](unmerged-work-to-main-or-deleted.md) carries
   every open PR, every branch, every dirty worktree and the stash to one of those
   two end states, todo by todo, with this section's gap register and container map
   as its measurements. A source PR closes only as a consequence: when every delta
   it carries is on main through a merged PR or deleted with its reason on main.
   W2-8 of that plan consumes the remaining C09 work and closes #81; no exception
   survives.
...
4. Cure the defects on main from the gap register in the order §Order and limits
   ...
   The hygiene-gate row's red test lands with the Q-07 relocation cure as the
   unmerged-work plan's W1-18 and W1-19; Q-07 then resolves its register row.
...
Consequences for containers: a source PR closes only when every delta it carries is
on main through a merged PR or deleted with its reason recorded on main; its branch
is deleted at that moment, since the PR keeps its head reachable. A red test on a
branch preserves nothing. A dirty worktree's files reach the same two end states.
There is no third state: no register row, queue row, retained branch or later
continuity commit carries a delta. The executable plan
[unmerged-work-to-main-or-deleted](unmerged-work-to-main-or-deleted.md) names the
todo for each container (owner rulings, 20 and 21 September).
...
| JSON Schema `$ref` siblings, draft-07 `additionalItems`, content keywords not carried round trip; `unevaluatedProperties`, `unevaluatedItems`, `dependentSchemas` and `prefixItems` are stripped silently on the draft-07 downgrade and before union-member conversion (`cloneWithoutSharedKeywords`)                                          | No content loss; roadmap JSON Schema commitment                                                                | observation                                                                                                                                                                                                                                                                                                   | #16, requires re-derivation against Q-04                                                                                                                                                                                                                                                   |
...
| Document-level `security: []` is lost by the OpenAPI writer: authored presence is not preserved while absence is (both authorise anonymous access)                                                                                                                                                                                             | No content loss                                                                                                | observation, reproduced by the 6 September probe (`PR18/document-security-empty` in the findings manifest)                                                                                                                                                                                                    | #18 security slice; C08/identity-interactions carrier                                                                                                                                                                                                                                      |
| An `x-` extension under a path item is read as a path or operation                                                                                                                                                                                                                                                                             | No content loss; extensions preserved                                                                          | observation; red test retained on #18 (`builder.operations.unit.test.ts`, run on unpatched main 20 September)                                                                                                                                                                                                 | #18                                                                                                                                                                                                                                                                                        |
...
| #21                                           | logger, runner, scanner                  | logger landed in #100; the Q-07 worktree E2E test passes 12/12, which is post-application state and not proof                                                                                                                                                                                                       | #21 closes at the unmerged-work plan's W1-19 once the lint restriction lands with the relocations (W1-18, W1-19); the scanner, baselines and timeout widening are deleted there, reason: the lint restriction is the gate                 |
| #23                                           | Practice transplant                      | 21 files conflict; main is ahead on shared parts                                                                                                                                                                                                                                                                    | no product gap; closes at the unmerged-work plan's W2-1 after each unique delta is read against main and the Practice home and landed or deleted; the branch is deleted then (the 21 September ruling supersedes the 26 August retention) |
| #81                                           | documentation source                     | 9 of 136 records applied                                                                                                                                                                                                                                                                                            | closes at W2-8 (C09's remaining work is W2-6 and W2-7); the `07-test-hygiene.md` numbering defect lands in W2-6's `.agent/plans` group                                                                                                    |
...
| custody-corrections, operation-security-empty | none; content already on main            | code files equal main                                                                                                                                                                                                                                                                                               | deleted at W4-4 (reason: content already on main)                                                                                                                                                                                         |
| fixture-reproduction                          | stale fixtures                           | generated, unformatted, carries the undecided integer output                                                                                                                                                                                                                                                        | deleted at W4-4 after W1-17 regenerates the fixtures (reason: generator output)                                                                                                                                                           |
...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `stash@{0}` on main at `e025d233`   | Five added lines in `.codex/config.toml`: `web_search`, `sandbox_mode` and `[sandbox_workspace_write] network_access`; none of them on main; no product content                        | Landed as the unmerged-work plan's W4-3, where the surface question is answered (the owner ruled on 13 September that Codex needs code execution and network access in some contexts and that this is not an owner decision); dropped by the owner once that PR has merged |
| Local branches without an upstream  | 27, including the source-PR branches and the `worktree-wf_*` branches; `git log --branches --not --remotes` is empty, so every commit is on `origin`                                   | Deleted at W4-1 (empty diff) or with their PRs (W1, W2)                                                                                                                                                                                                                    |
| Worktree registrations              | 37 registered on 20 September; two prunable: `castr-q07-zod-fixture-runner` and Codex's `castr-q07-test-isolation`, both under the system temp directory with their directories purged | Pruned at W4-1; nothing on disk to preserve                                                                                                                                                                                                                                |
...
own example; #81 stays open until C09 (W2-8); #23 closes at W2-1. Owner decisions
are taken on their PRs: the vision doctrine text (W2-3), any removed CLI option wanted
back as a feature (W1-13).
...
record, #81 through C09; every dirty worktree's files are merged or deleted with the
reason recorded on main and the worktree removed; no unpushed work remains, the
stash included; the ledger and this section agree with
