...
| #21                                           | logger, runner, scanner                  | logger landed in #100; the Q-07 worktree E2E test passes 12/12, which is post-application state and not proof       | red test on unpatched main: the in-process runner reads the filesystem and `process.env`, caught by the lint restriction row; land that red test with the relocation cure; then close; the scanner is superseded by the lint restriction |
...
| q07-zod-fixture-runner                        | runner FS I/O in process                 | E2E green in place; one file unformatted; no red on unpatched main recorded yet                                     | add the lint-restriction red test on main, commit with hooks, PR, merge                                                                                                                                                                  |
