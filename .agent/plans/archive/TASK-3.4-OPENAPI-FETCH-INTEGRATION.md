...
```
┌─────────────────────────────────────────────────────────────┐
│  Generated API Client (Our Code)                            │
│  - Type-safe wrapper methods (getPet, createUser, etc.)     │
│  - Zod validation for requests & responses                  │
│  - Error handling with Zod parse failures                   │
└─────────────────┬───────────────────────────────────────────┘
                  │ uses
┌─────────────────▼───────────────────────────────────────────┐
│  openapi-fetch (Peer Dependency)                            │
│  - HTTP client with OpenAPI-aware types                     │
│  - Path parameters, query strings, headers                  │
│  - Content negotiation (JSON, form-data, etc.)              │
└─────────────────┬───────────────────────────────────────────┘
                  │ uses
┌─────────────────▼───────────────────────────────────────────┐
│  openapi-typescript (Peer Dependency)                       │
│  - Compile-time TypeScript types from OpenAPI              │
│  - Type inference for paths, methods, parameters            │
└─────────────────────────────────────────────────────────────┘
```
...
- ✅ **Battle-tested HTTP logic** - Don't reinvent the wheel
- ✅ **Minimal code generation** - We just add validation wrapper
- ✅ **Type safety at compile time** (openapi-typescript) + runtime (Zod)
- ✅ **Maintenance burden stays low** - HTTP logic maintained by openapi-ts team
- ✅ **Standards compliant** - openapi-fetch follows OpenAPI spec closely
- ✅ **Small bundle size** - openapi-fetch is lightweight

**Cons:**

- ⚠️ **Two peer dependencies** - Users must install openapi-fetch + openapi-typescript
- ⚠️ **Learning curve** - Users need to understand openapi-fetch API
- ⚠️ **Version compatibility** - Must keep in sync with openapi-fetch releases
...
export function createApiClient(config: ApiClientConfig) {
  ...
  return {
    ...
    async getPetById(params: { petId: string }) {
      ...
      // 4. Validate response
      const validated = validate(Pet, data, 'getPetById response');
    ...
    async addPet(body: z.infer<typeof CreatePetRequest>) {
      ...
      const validatedBody = validate(CreatePetRequest, body, 'addPet request body');
      ...
      // 4. Validate response
      const validated = validate(Pet, data, 'addPet response');
...
### Phase 3: REFACTOR - Polish & Document (60 min)

#### 3A: Inline Documentation (20 min)

1. **Comprehensive TSDoc for all exports:**
   - Full JSDoc for `createApiClient()` function
   - Examples in every method (with @example tags)
   - Parameter descriptions with @param
   - Return type documentation with @returns
   - Error scenarios with @throws
2. **Configuration options documentation:**
   - Document all ApiClientConfig fields
   - Provide examples for each option
   - Document validation modes (strict/loose/none)

#### 3B: Authored Markdown Documentation (30 min)

Create **comprehensive** documentation files:

1. **`docs/USAGE.md`** - Complete usage guide
   - Installation steps (all peer dependencies)
   - Quick start examples
   - All template options (schemas-only, schemas-with-metadata, schemas-with-client)
   - Configuration examples
   - Error handling patterns
   - Advanced use cases

2. **`docs/OPENAPI-FETCH-INTEGRATION.md`** - Deep dive on new template
   - Why openapi-fetch?
   ...
   - Migration from Zodios (if applicable)

3. **`docs/EXAMPLES.md`** - Extensive examples
...
#### 3C: Testing & Verification (10 min)

1. **Add error handling tests**
2. **Verify with real OpenAPI specs**
3. **Validate documentation examples compile**
...
4. **No Zodios:** Zero references to `@zodios/core`
5. **Type safe:** Generated code compiles with no type errors
6. **Runtime validated:** Zod validation on requests and responses
7. **Peer deps:** Documented requirement for openapi-fetch + openapi-typescript

### Documentation Quality

8. **Inline TSDoc:** Every export has comprehensive JSDoc with examples
9. **Usage guide:** `docs/USAGE.md` with all templates and options
10. **Integration guide:** `docs/OPENAPI-FETCH-INTEGRATION.md` with architecture
11. **Examples:** `docs/EXAMPLES.md` with extensive real-world examples
12. **API reference:** `docs/API-REFERENCE.md` with complete type signatures
13. **README update:** Main README.md updated with new template option
14. **Migration guide:** Documentation for users migrating from Zodios
15. **Compilable examples:** All code examples in docs must compile
