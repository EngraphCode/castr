...
todos:
  ...
  - id: C4
    ...
    status: completed # RE-OPENED then RESOLVED-via-LC1 2026-06-27. RE-OPENED (Stratospheric Kiting Breeze): the "code primitives present" premise was DISPROVEN — the F-95 presence-GATE layer was ABSENT, detectStaleWatcher dead. RESOLVED (Hidden Veiling Mirror, `6372024`): LC1 brought the full gate (claims-open-watcher-gate, watcher-presence, assert-watcher-live, watcher-staleness-io), wired it into openClaim, and gave detectStaleWatcher a production caller. Authoritative home: practice-loop-closure-remediation.md § As-built (LC1).
  - id: C5
...
| --- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A1  | ArcAngel / ARC rapid-comms protocol ✅ DONE                           | `.agent/reference/arc-rapid-communication.md` (354L) + `.agent/collaboration/rapid-comms/`                                                | ✅ DONE 2026-07-18 (`feat/arc-rapid-comms-bring`) — FULL UNIT via Resonance's current estate: reference doc + collaboration homes + format-gate guards + grammar/validator/colour CLI + watcher-pairing clauses + commit ride-along + statusline feather wing; first live channel opened same-day (owner-directed trial) | M    | Owner-named. Complementary low-latency peer-dialogue channel; complements (not replaces) canonical comms. Brought via Resonance (which ported Oak pin f556b08de) — the living estate had evolved well past the Oak snapshot (schema-gated structure, colour ceremony, feather badges).                                   |
...
| A4  | statusline session-shape indicators ✅ DONE                           | `statusline-session-shape.ts` (219L), `statusline-indicators.ts`, `statusline-ansi.ts` + `--role`                                         | ✅ DONE — session-shape indicators landed pre-2026-07-18 (arcActive wing socket); the full ARC feather layer (per-channel palette-ink badges, bounded channel reads, roster-union membership, observing-directed shape) landed 2026-07-18 with A1 on `feat/arc-rapid-comms-bring`                                        | M    | Team-shape/director/ArcAngel-wing, fed by claims+rapid-comms reads. `oak-logo.ts` is a brand asset → DON'T bring. Attention + identity-prefix layers deliberately remain upstream (own lanes; replication paths in Resonance's transfer report).                                                                         |
| A5  | reason skill + grammar-of-thinking (thinking-discipline pair) ✅ DONE | `.agent/skills/cognition/reason/SKILL-CANONICAL.md` + `.agent/reference/grammar-of-thinking.md` (1432L)                                   | ✅ DONE 2026-06-26 (`4f0bfe3`) — brought from Oak live `main` + localised; `metacognition` back-link completes the pair (now byte-identical to Oak)                                                                                                                                                                      | M    | Newly-found gap, post-dates the 2026-06-20 audit. Outward structured-thinking pair to castr's inward `metacognition`. `engraph-reason` adapter generated + discoverable; portability now 19 skills. `citation-as-reasoning` pattern was already-present (stale BRING disposition). Plan: `reason-skill-parity-bring.md`. |
...
| C4  | watcher liveness self-check + mutual-cover  | ✅ RESOLVED-via-LC1 2026-06-27 (`6372024`): F-95 presence-GATE layer BROUGHT + wired into openClaim; detectStaleWatcher now has a caller — see loop-closure plan § As-built (LC1) | S      |
