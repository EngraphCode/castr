...
- **the proof-programme parent plan IS the plan-of-record (W-0 walked and CLOSED, 2026-08-22):** the owner walked the [W-0 ballot](./proof-programme/ballot-2026-08-owner-walk.md) interactively — all ten decisions carry success verdicts, and **ballot item B-11 RATIFY supersedes the 2026-06-19 sequence above with [`proof-programme/parent-plan.md`](./proof-programme/parent-plan.md)'s queue** (the transplant pauses as a named position with re-entry record QD-2; both former active-lane plans now sit in `current/paused/` with disposition banners). [ADR-051](../../docs/architectural_decision_records/ADR-051-autonomous-background-implementation-loop.md) is Accepted (amended: three firings/day). Next: Q-01 (loop readiness, arms the Routine), then the queue per the parent plan's operating protocol
