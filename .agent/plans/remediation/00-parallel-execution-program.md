...
| New findings from PR reviews (routed, 2026-07-18) | MCP nullability fold (L-K10, from PR #20); Zod-writer carried `$ref` siblings (L-B, from PR #16 thread 8); prototype-unsafe assignment seams beyond L-D's sweep (`openapi-writer.components.ts` result maps; `conversion/json-schema/keywords/keyword-helpers.ts` `setKeyword`; `ir/models/schema.ts` `CastrSchemaProperties.get/has`) — candidate horizontal micro-lane; `turbo.json` test-task missing `outputs` warning (flagged by three lanes; no-warning-toleration) — candidate config micro-lane; `x-*` specification-extension capture (PR #18 round: extensions are dropped estate-wide by the closed-shape IR convention at EVERY level — document, path item, operation, response, parameter, schema; true losslessness needs an IR-model + writer extension bag across all of them, and partial capture at one level would be inconsistent — candidate IR-fidelity backlog lane, sits beside the flat-`IRSecurityRequirement` class); the remaining named positions (L-C primitive-parser `.describe()` drop, `.meta()` unknown-key drop, `length(n)`/regex-flag gaps; L-H `outputSchema` derivation gap; L-F `cloneWithoutSharedKeywords`) are carried in the thread record §Pending decisions until dispositioned |
