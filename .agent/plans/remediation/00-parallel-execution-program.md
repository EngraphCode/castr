...
- **Owned:** C2 — `src/schema-processing/parsers/openapi/operations/fields/builder.operations.fields.ts`,
  ...
  **Addendum (2026-07-18, orchestrator):** the PR #18 prototype-safety class sweep extended
  in-lane to `context/mcp/schemas/template-context.mcp.inline-json-schema.ts` (+ its from-ir
  test) — entries-assembly hunks in commit `1a2058d8`. Interaction notes: L-E's #20 diff touches
  the same file (guard consolidation; L-D rebases over L-E's merge per wave order), and L-K10's
  later nullability fold must preserve the entries-assembly shape. Remaining prototype-unsafe
  seams OUTSIDE the sweep are named in the routed-findings disposition row.
...
- **L-K9** (M9 representable-mapping remainder; added 2026-07-17 from L-K1's PR review — the
  ...
  drops its unimplemented-mapping fail-fast for TS. **Starts after L-KBATCH AND L-D merge** (L-D
  owns `writers/typescript/helpers.ts` inside L-K9's `writers/typescript/**` surface — PR #10
  round 11 caught the missing gate; shared
...
| New findings from PR reviews (routed, 2026-07-18) | MCP nullability fold (L-K10, from PR #20); carried-`$ref`-siblings silent drop in BOTH codegen writers — Zod (L-B, from PR #16 thread 8) AND the TypeScript writer's `writeRefType` short-circuit (P1, PR #16 wind-down round; shared cure shape: carried-siblings-aware reference emission or honest fail-fast, decided against the input-output pair model); prototype-unsafe assignment seams beyond L-D's sweep (`openapi-writer.components.ts` result maps; `conversion/json-schema/keywords/keyword-helpers.ts` `setKeyword`; `ir/models/schema.ts` `CastrSchemaProperties.get/has`) — candidate horizontal micro-lane; `turbo.json` test-task missing `outputs` warning (flagged by three lanes; no-warning-toleration) — candidate config micro-lane; `x-*` specification-extension capture (PR #18 round: extensions are dropped estate-wide by the closed-shape IR convention at EVERY level — document, path item, operation, response, parameter, schema; true losslessness needs an IR-model + writer extension bag across all of them, and partial capture at one level would be inconsistent — candidate IR-fidelity backlog lane, sits beside the flat-`IRSecurityRequirement` class); the remaining named positions (L-C primitive-parser `.describe()` drop, `.meta()` unknown-key drop, `length(n)`/regex-flag gaps; L-H `outputSchema` derivation gap; the `.default()`-implies-required parser convention (PR #13 wind-down round: Zod accepts an omitted key and supplies the default, so `metadata.required=true` misrepresents the INPUT contract — but the convention is repo-wide and deliberate, so the cure is a design ruling on input-optionality vs output-presence semantics, not a lane patch); L-F `cloneWithoutSharedKeywords` — UPGRADED to guard-bypassing with a concrete instance: `{type:['object','null'], unevaluatedProperties:false}` routes through `convertUnionSchema`'s clone-strip before `applyObjectKeywords`'s fail-fast ever runs, PR #16 wind-down round) are carried in the thread record §Pending decisions until dispositioned |
