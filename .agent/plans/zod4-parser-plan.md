...
```text
OpenAPI → IR → Zod → IR → OpenAPI (true round-trip)
```
...
| Zod 3 Pattern (❌ Reject) | Zod 4 Equivalent (✅ Accept) |
| ------------------------- | ---------------------------- |
...
| `z.number().int()`        | `z.int()`                    |
| `z.union([A, B])`         | `z.xor([A, B])` for oneOf    |
...
Use **ts-morph** to parse the TypeScript AST:

```text
Source File → ts-morph AST → Pattern Recognition → IR Construction
```

**Key challenges:**

- Getter syntax for recursive references
- `.meta()` → IR metadata mapping
- `.strict()` → `additionalProperties: false`
- Format function recognition (`z.email()` → `format: 'email'`)
...
├── zod-parser.primitives.ts  # Primitive recognition
├── zod-parser.object.ts   # Object schema parsing
├── zod-parser.composition.ts # Union/intersection
└── zod-parser.validation.ts  # Zod 3/4 detection, error generation
```
