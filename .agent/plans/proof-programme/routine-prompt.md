...
1. **STOP check (exact path, before anything opens a claim)**: if the file
   ...
   the kill switch permits (QD-5), routed contest-aware exactly like step 3's deferrals: a
   bookkeeping-scope commit on the open programme PR's head only when that head is
   uncontested under step 4's overlap guard; when it is contested, or no programme PR is
...
2. **Ground**, in this order:
   ...
   - Ground with the `engraph-start-right-thorough` skill (owner ruling, 2026-08-26:
     ...
     through step 6's queued-decision branch, exactly like one found during execution. The session's
     ...
     boundaries and whenever a rabbit hole or fluent shortcut appears. Wherever the
     stored trigger prompt's wording and this brief ever differ, THIS BRIEF GOVERNS —
     the stored prompt is a thin pointer whose own opening instruction is to follow
     this file exactly: firings ground thorough here, and close under step 8's
     firing-scoped `session-handoff` profile, recording any deeper consolidation as due
     rather than running the `wrap` deep-close inside a duration-bounded firing.
   - **Owner-decision surfacing**: read [`queued-decisions.md`](./queued-decisions.md)
     and carry every entry whose Outcome is OPEN into this firing's step 8 completion
...
3. **Claims scan**:
   ...
   programme PR is already open** (step 4's WIP = 1 guideline binds here too — do not open a
   ...
   **contested** (overlap guard, step 4), never write to its head — land the increment and
   ...
   contested ref), for pickup per step 4's draft-pickup rule. Before incrementing, read
...
4. **WIP = 1 — every open non-draft programme PR counts** (a guideline served with
   ...
   record via step 3's contested route (the shared deferral draft — never a push to the
   ...
   one merge carries both firings' counter state — "do nothing else" never waives step 7's
...
6. **Branches** (each is normal operation, not an error):
   ...
   - **Genuine owner fork**: write it to [`queued-decisions.md`](./queued-decisions.md)
     ...
     claims are your claims (a 2026-08-26 owner re-read measured five of eight
     ...
     QD row so any later firing reads the answers there — where this session can publish
     artifacts and deliver a push. Where it
...
8. **Close**: run the `engraph-session-handoff` skill under the **firing-scoped profile
   ...
   still OPEN and awaiting the owner (from the step 2 read — an empty register is stated
...
ruling stands, and anything durable still lands as tracked repo state. When the Slack
tools are absent from this session, proceed without them and name the absence in the
completion summary.
