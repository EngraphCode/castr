...
owner_directive: >-
  ...
  the Practice has largely operated in local, interactive sessions, now it is
  moving into cloud sessions and routines as well, and we have discovered we
...
The Practice operates in cloud sessions and scheduled Routines with the same
trustworthiness it earned in local interactive sessions: an autonomous seat's
claims about itself and its world are reliable enough that the owner's
attention is spent on genuine forks, not on catching relayed errors. The value
currency is **trust per unit of owner attention** — a landed slice the owner
must re-check is worth little; a landed slice the owner never has to think
...
The 2026-08-26 arming walk was the estate's first live traversal of the
account-side machinery, and it returned a split verdict that defines this
node's work:

- **Mechanical trust is earned.** Gates fired, the dry-run seat held
  read-only, byte-verification caught a real stored-prompt corruption, the
...
- **Epistemic trust failed on first owner contact, everywhere tested.** The
  ...
  no measurement backed: the npm registry shows no current release). The
  notification channel: configured since 2026-08-23, never actually delivered
  until the owner repaired the account config mid-walk. The model setting:
  recorded as set, first measured firing served a different model. The
  walking session itself reproduced the same failure class live by relaying
  register rows unverified — and the 2026-08-27 session reproduced it again
  (assumed hold reasons, an interpreted receipt gate, an unratified wording)
  until its own metacognition pass caught the pattern.

The bet: the cure is not more machinery but **grounded cognition made
mandatory in the environment where no owner is watching** — cloud seats
carry the full discipline stack, and every loop-authored owner-facing surface
carries a verified-premise, owner-plain-language contract. Configuration is
never trusted as behaviour: only a live traversal proves an account-side
mechanism, and each mechanism's first traversal is treated as its real test.

We are deliberately **not** doing: more platform machinery ahead of need; a
parallel-workers design; per-session-type exceptions to Practice mechanisms
(one policy serves every medium — the medium is a parameter, not a new
category, per the owner's 2026-08-24 teaching).

## The autonomous Practice agent

Owner direction (2026-08-27): the Routine's configuration — the stored
trigger prompt and every trigger setting — is explicitly part of **creating
an autonomous Practice agent**, not incidental platform state. The agent's
definition spans three layers, each repo-governed:
...
2. **Structure and support** — the fired agent's cognition is repo-carried:
   ...
   kill switches.
3. **Learning across sessions** — the agent's memory is the repo. Write side:
   per-firing napkin capture, thread-record identity rows, evidence records,
   and the incident register (the firing-scoped `session-handoff` profile,
   QD-6). Read side: every firing's thorough grounding. The loop is closed by
   the **curation drain** — the consolidation Routine (the routine-config
   session's Act 3, estate pending) that graduates firing captures into
   permanent homes, so later firings inherit lessons as doctrine rather than
   as an unread backlog. Corrections encoded structurally rather than
   re-taught is this node's success criterion; the drain is its mechanism.

## Already landed

2026-08-26 walk instalments:
...
- Cloud sessions ground with `start-right-thorough` plus the mandatory
  ...
  cloud-environment.md §Cloud-session grounding contract.
- The queued-decisions row contract: rows are written for the owner in plain
  ...
  routine-prompt owner-fork branch).
- The `never-edit-generated-files` rule (owner-directed, QD-11).
...
- A firing's self-report (completion summary, incident rows, register rows,
