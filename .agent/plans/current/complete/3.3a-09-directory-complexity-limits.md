...
- **[NEW]** `lib/eslint-rules/max-files-per-dir.ts`: A custom ESLint rule that enforces a maximum number of files in a directory (non-recursive).
  ...
  - Applies the `.agent/directives/principles.md` disciplines (No `any`, type guards explicitly over assertions).
...
- [ ] Create `max-files-per-dir` custom ESLint rule.
  ...
  - [ ] Ensure strict adherence to `.agent/directives/principles.md` (no assertions, no any).
