...
Repo-wide violations (265 across 63 non-parser files) are owned by [3.3a.04](./3.3a-04-centralize-data-string-parsing.md).
...
2. **Read** semantic-parsing-issues-initial-2026-02-14.md (historical report citation: semantic-parsing-issues-initial-2026-02-14.md; the report is absent from the current checkout and is not current verification evidence) — the full violation inventory. Priorities 1, 2, and 4 contain the violations this plan owns.
3. **Read** [principles.md](../../../directives/principles.md) — TDD, fail-fast, type discipline. No fallbacks, no escape hatches, no compromises.
...
**Begins after:** [3.3a.02 — ESLint Enforcement Redesign](./3.3a-02-eslint-enforcement-redesign.md) (✅ complete — lint rules active, violations flagged)  
...
**Successor:** [3.3a.04 — Repo-Wide ADR-026 Remediation](./3.3a-04-centralize-data-string-parsing.md)  
**Cross-session dependency:** [3.3b.02 — Scenario 3](./3.3b-02-scenario3-reference-composition.md) depends on the symbol table built here.
...
1. **No `any` — even in synthetic stubs**: `createZodProject()` uses `export declare const z: any;` in the synthetic zod declaration file. `any` is forbidden per principles.md, no exceptions. **Fix:** Replace with a structurally typed declaration (Phase 1b).
...
> [!IMPORTANT]
> **Previous attempt (reverted):** Created `ir-text-utils.ts` with `isZodExpression` using `startsWith('z.')` and `isSuccessStatusCode` using `startsWith('2')`. Both were text heuristics. `eslint-disable` was used to suppress the lint rule. This violated ADR-026 and principles.md. The file has been **deleted** and all imports reverted to original inline code. The violations below must be solved properly.
...
> [!NOTE]
> Remaining repo-wide ADR-026 lint violations are owned by [3.3a.04](./3.3a-04-centralize-data-string-parsing.md).
...
- semantic-parsing-issues-initial-2026-02-14.md (historical report citation: semantic-parsing-issues-initial-2026-02-14.md; the report is absent from the current checkout and is not current verification evidence) — full violation inventory
