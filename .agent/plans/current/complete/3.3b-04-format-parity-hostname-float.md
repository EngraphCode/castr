...
> ⚠️ **Misfiled (2026-06-04):** marked "🔄 Active" yet filed in `current/complete/`. The hostname/float format-parity
> work is recorded complete in `roadmap.md`. Reclassify: if landed, set status Complete and archive; if not, move to
> `current/paused/`. (Not independently re-verified in this review pass.)
