# Plan: 3.3b.02 — Scenario 3: Reference-Based Composition Parsing

**Status:** 🔲 Not Started  
**Priority:** High  
**Created:** 2026-02-13  
**Last Updated:** 2026-02-16  
**Parent:** [roadmap.md](../../roadmap.md) (Session 3.3b)

---

## Goal

Fix Scenario 3 by teaching the Zod parser to recognize compositions that start from a schema identifier reference, not `z.`:

- `NewPet.and(z.object(...))`
- `Foo.merge(z.object(...))`
- `Bar.extend({ ... })` (if used/emitted)

---

## Responsibility Boundary

**Begins after:** [3.3b.01 — Round-Trip Suite Strictness](3.3b-01-roundtrip-suite-strictness.md)
**This plan owns:** Extending Zod parser detection to handle identifier-based compositions. Making Scenario 3 pass strict assertions.
**This plan does NOT own:** The symbol table architecture itself (that's built in 3.3a.03). This plan CONSUMES the symbol table.
**Cross-session dependency:** Requires [3.3a.03 — Zod Parser Semantic Parsing](../complete/3.3a-03-zod-parser-semantic-parsing.md) to be complete (symbol table must exist).
**Successor:** [3.3b.03 — Reject z.undefined()](3.3b-03-reject-z-undefined.md)

### Preconditions (verify before starting)

- [ ] Plan 3.3a.03 is marked ✅ Complete in [roadmap.md](../../roadmap.md)
- [ ] Symbol-table resolver exists and is tested (e.g., `zod-import-resolver.ts`)

---

## Repo Truth

- Zod writer emits reference-based intersections for `allOf`:
  - `export const Pet = NewPet.and(z.object(...))`
- Zod parser detection currently only recognizes declarations starting with `z.`:
  - `lib/src/schema-processing/parsers/zod/zod-parser.detection.ts`

---

## Work

1. Track declared schema names during parse (symbol table from 3.3a.03).
2. Treat `Identifier.and(...)` (and related composition methods) as parseable when `Identifier` is a known schema.
3. Add fixtures and round-trip tests proving:
   - schema count is preserved strictly,
   - re-parsed OpenAPI output matches original strictly.

---

## Acceptance Criteria

- Scenario 3 assertions in `round-trip.integration.test.ts` are strict equality.
- `petstore-expanded-3.0.yaml` round-trips through Zod layer without schema loss.

---

## References

- `lib/src/schema-processing/parsers/zod/zod-parser.detection.ts`
- `lib/tests-roundtrip/__tests__/round-trip.integration.test.ts`
- `.agent/plans/roadmap.md` (Session 3.3a; ADR-026 interaction)
