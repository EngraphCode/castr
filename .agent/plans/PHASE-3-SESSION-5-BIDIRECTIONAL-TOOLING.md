...
2. **Semantic Equivalence:** The generated OpenAPI spec might not be byte-for-byte identical (ordering, whitespace), but it must be *semantically* identical to the input.
...
- **[ ] Fidelity Test Suite**
  ...
  - For each example spec (TicTacToe, Petstore, etc.):
        1. Parse Original (`OpenAPI_1`).
        2. Build IR (`IR_1`).
        3. Generate OpenAPI (`OpenAPI_2`).
        4. Compare `OpenAPI_1` and `OpenAPI_2`.
