# @engraph/eslint-plugin-standards — Detailed Plan and Architecture

## Overview

This document describes the full plan for building the **@engraph/eslint-plugin-standards** package — a reusable, modular ESLint plugin that encapsulates @engraph's internal linting standards for TypeScript, React, and Next.js projects.
...
```json
{
  "name": "@engraph/eslint-plugin-standards",
  "version": "1.0.0",
  "type": "module",
  "main": "./dist/index.js",
  "files": ["dist"],
...
```bash
pnpm add -D @engraph/eslint-plugin-standards
```
...
```js
export default [
  { extends: ['plugin:@engraph/standards/recommended'] },
  { extends: ['plugin:@engraph/standards/react'] },
  { extends: ['plugin:@engraph/standards/next'] },
];
```
