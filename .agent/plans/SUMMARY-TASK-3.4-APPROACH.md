...
└──────────────┬──────────────────────────┘
               │ calls
┌──────────────▼──────────────────────────┐
│  Generated Wrapper (We Generate This)   │
...
└──────────────┬──────────────────────────┘
               │ uses
┌──────────────▼──────────────────────────┐
│  openapi-fetch (User Installs)          │
│  - Handles HTTP (GET, POST, etc.)       │
│  - Path params, query strings           │
│  - Content types (JSON, form-data)      │
│  - Uses openapi-typescript types        │
└──────────────┬──────────────────────────┘
               │ provides types
┌──────────────▼──────────────────────────┐
│  openapi-typescript (User Generates)    │
│  - Compile-time TypeScript types        │
│  - Type inference for all endpoints     │
└─────────────────────────────────────────┘
```
...
- ❌ Content type negotiation (JSON vs form-data vs multipart)
...
**Result:**
- 📝 500+ lines of generated code per endpoint
- 🐛 We maintain complex HTTP logic
- 🔧 Users file bugs against our HTTP client
- 🚀 Slow iteration (any HTTP bug requires generator update)

### After (Using openapi-fetch)

**What we generate:**
- ✅ Zod validation wrapper (~50 lines per endpoint)
- ✅ Call to openapi-fetch
- ✅ Error handling
- ✅ Type exports

**Result:**
- 📝 ~100 lines of generated code per endpoint
- ✨ openapi-ts team maintains HTTP logic
- 🎯 We focus on validation (our core value)
- 🚀 Fast iteration (HTTP bugs fixed upstream)
...
Both are:
- ✅ Actively maintained by OpenAPI TypeScript team
- ✅ Well-documented
- ✅ Widely used (7.5k+ stars)
- ✅ Lightweight
...
**Issues:**
- ❌ Tight coupling to Zodios
- ❌ Zod 4 incompatibility
- ❌ Limited to Zodios's HTTP client
- ❌ Can't customize fetch behavior
...
- ✅ Full Zod 4 support
- ✅ Can access raw client for customization
...
- ✅ New template: `schemas-with-client.hbs`
- ✅ Generates `createApiClient()` function
- ✅ Wraps openapi-fetch with Zod validation
- ✅ All tests pass (unit + integration + snapshot)
