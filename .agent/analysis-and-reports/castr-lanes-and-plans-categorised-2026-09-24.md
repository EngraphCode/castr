...
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| SPEC-N-1 model structure            | T-02B, T-02C and T-03 (value and interaction roots; accepted and produced facets; wire identity); PK-17 (references carried as keyword strings, which SPEC-AR-1 overrules); PK-30, PK-34 and PK-39 (keywords and Scalar OpenAPI types as model structure); QD-9 (persistence versioning, §4); PP-2 (flat security) |
| SPEC-N-2 seams                      | PK-20 (ADR-043, which hands the compiler split to ADR-048); PF-2 (verb and surface architecture); RS-4 (atomisation gate)                                                                                                                                                                                          |
...
| Proof standard (SPEC-G-5, SPEC-G-7) | Q-02 (mutant-bite runner with a source-oracle leg, first used at MEASURE); PK-44 (proof honesty); PK-18 (gates green while the fidelity suite was red)                                                                                                                                                             |
...
- **Leg** is the first leg, in course order, that would use the row's substance, for either of
  its categories, or `none`. §2 also names rows as context, such as a cost on a RATIFY card or an
  example off the corpus; a mention is not a use.
...
| Q-02 | Q-02 harness extraction (mutant-bite, non-vacuity) | completed   | done       | A       | B         | supports G-5 and G-7; its source-oracle runner is the closest built instrument for MEASURE; travels only if MEASURE says evolve | MEASURE | `.agent/plans/proof-programme/parent-plan.md` frontmatter                   |
...
text = REPORT.read_text(encoding='utf-8').split('<!-- register:start -->')[0]
row_id = re.compile(r'\b(?:LN|CL|PS|PA|PC|PP|PF|RM|PG|PT|PK|PX|PO|RG|RS)-\d+\b|\bQ-\d{2}\b|\bT-\d{2}[A-Z]?\b')
register_ids = {c[0] for _, c in rows}
print('IDs named before the register but absent from it:', sorted(set(row_id.findall(text)) - register_ids) or 'none')
print('D rows with a secondary category:', [c[0] for _, c in rows if c[4] == 'D' and c[5] != '—'] or 'none')
```
...
IDs named before the register but absent from it: none
D rows with a secondary category: none
```
