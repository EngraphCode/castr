...
out_, _exception to_, _for these arcs_, _honest framing for X_,
_permitted variant_, _land it then iterate_, _cheap cure_,
_good enough_, _quick fix_ — is the same failure shape in different
dressing. Vocabulary is not the trigger; _intent_ is. If the
...
- `principles.md` and `distilled.md` — these documents _catalogue_
...
3. **Sequence honestly.** A _sequenced deferral_ names the plan and
