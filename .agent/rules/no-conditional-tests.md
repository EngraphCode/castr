...
Operationalises [`testing-strategy.md`](../directives/testing-strategy.md) (castr's authoritative test methodology). Origin: Oak ADR-011 (Use Vitest) and ADR-078 (Dependency Injection for Testability) — cross-host; castr's ADRs with those numbers are unrelated decisions. Sibling to [`no-skipped-tests.md`](no-skipped-tests.md).
...
- `architecture-expert-fred` (principles-first) flags product-code shapes that _force_ test authors toward conditionals — multiple-mode functions, env-detection inside libraries, ambient-state coupling — as architectural-failure signals at the source.

## Cross-references

- Authority: [`testing-strategy.md`](../directives/testing-strategy.md) — deterministic, behaviour-proving tests.
...
- Operational checklist: [`test-immediate-fails.md`](test-immediate-fails.md) — fast-gate rejection list for the test reviewer.
- Origin doctrine: Oak ADR-078 (dependency injection for testability; cross-host).
