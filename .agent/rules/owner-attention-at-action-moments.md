...
at promotion: _"if the framing is useful let's try it, if it doesn't work
we can always change it"_. The rule is in force; agents should apply it.
...
- **Action-moment surfacing**: surface the irreversibility explicitly —
  _"about to commit X / push Y / send Z to peer agent A"_ — so the owner
