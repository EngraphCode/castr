...
[`quality-gate-failures.md`](quality-gate-failures.md) — all gate failures
are blocking, regardless of location, cause, or context). Anti-pattern
named: **`gate-off-fix-gate-on`**.
...
- **Adding `eslint-disable` / `eslint-disable-next-line`** to
  silence a rule. (Oak enforces this structurally via its
  ...
  custom ESLint rule — cross-host; castr enforces it at review time, and
  the castr `lib/eslint-rules/` family is the natural home for a
  structural equivalent.)
- **Adding `// @ts-expect-error` / `// @ts-ignore`** to bypass a
  type error. Fix the type flow upstream — see
  [type-reviewer](../sub-agents/templates/type-reviewer.md) and castr's
  type-system discipline in `principles.md`.
...
- A test in any test suite (`pnpm test`, `pnpm test:e2e`,
  `pnpm test:snapshot`, `pnpm test:gen`, `pnpm character`, etc.).
...
- Authority: [`DEFINITION_OF_DONE.md`](../directives/DEFINITION_OF_DONE.md)
  and [`quality-gate-failures.md`](quality-gate-failures.md) — all gate
  failures are blocking; never disable, never work around, never weaken
  a gate to solve a testing problem.
