...
| ----------------------------------------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "Throw out my draft cycle-1 code"         | `git checkout HEAD -- <files>` | `rm <new files>`, then `Edit` the modified files to remove the changes you no longer want, leaving the _kept_ parts                                        |
...
same files. Owner correction: _"we don't throw away work, we remove it,
we go forward not backwards, change the files, don't use git."_
