...
It is R2 of the origin estate's WS0 skills-estate recommendation
(OCE, `.agent/reports/agentic-engineering/skills-estate-organisation-ws0-recommendation.md`),
adopted there at the owner's full ruling 2026-08-02 and transplanted
here 2026-08-24 under the owner's equality directive.
...
Verbatim (origin-estate ruling; "Oak skills" reads here as the
estate's own in-repo skills): "no skills should be vendored, we have
Oak skills, we have skills installed with `npx skills add` or
`pnpm skills add` that is it." Exactly two legitimate skill channels
exist: **estate-authored skills in-repo**, and **installer-managed
Vendor skills** via
...
1. **Audience set first.** Which audience does this capability
   ...
   future mechanisms (owner partition, adopted 2026-08-24 from the
   origin estate's 2026-08-02 ruling). This estate currently has one
   audience set; a capability that fits no existing set does not
   stretch one — a new set lands deliberately, recorded in the
   audience-set registry in
...
4. **Who recognises the trigger?** A trigger-bound landing is sound
   only when the _loader_ — a path match, a tool-call match, a
