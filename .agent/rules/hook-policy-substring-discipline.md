...
guards, or instructional framing — _"do not use X"_ and _"use X"_
trip the same filter when _X_ is on the trip-list.
...
agent-authored content that _describes_ a forbidden pattern in order
...
- whole-tree staging glob → _"whole-tree shortcut"_ or _"the staging
  glob shortcut that picks up every modified file"_
- the hook-bypass flag on `git commit` → _"the hook-bypass flag"_
  or _"the verify-skip flag"_
- environment-variable hook-skip mechanisms → _"environment-variable
  hook-skip"_
- `core.hooksPath=/dev/null` reroute → _"hook-redirect mechanism"_
...
_"Hook-policy substring-matching in instructive content is a
recurring blocker"_.
