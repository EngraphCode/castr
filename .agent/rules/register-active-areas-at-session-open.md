...
(documented in Oak's `parallel-track-pre-commit-gate-coupling` pattern —
cross-host origin)
...
The reactive discipline (_"surface, don't fix or bypass, route to owner"_)
is now well-recorded but does not prevent recurrence.

WS0 of Oak's multi-agent-collaboration-protocol plan (cross-host
origin) installed the shared communication log — now treated as a generated
...
rule structures the _area-of-work_ signal so subsequent agents do not have to
...
the WS1 implementation slice in `active-claims.json` _before_ staging the WS1
