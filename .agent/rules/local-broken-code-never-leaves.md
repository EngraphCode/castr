...
Operationalises the **local broken code never leaves** principle —
the publish-side face of castr's blocking-gate doctrine
([`DEFINITION_OF_DONE.md`](../directives/DEFINITION_OF_DONE.md),
[`quality-gate-failures.md`](quality-gate-failures.md)).
...
- _"It compiles"_ — compilation is necessary, not sufficient.
- _"The type-checker is happy"_ — same.
- _"The tests pass without me running the changed test"_ — the
  changed test must run.
- _"The lint passes"_ — lint is a syntactic property, not a
  behavioural one.
- _"I think it works"_ — observation, not opinion, is the proof.
...
- _"But it's just a typo fix"_ → still build it and observe it
  rendering / running.
- _"But it's just a doc change"_ → markdown lint + render check is
  the local proof; it's cheap, run it.
- _"But it's just a comment"_ → no proof required if truly only
  comments; verify the diff is comment-only.
- _"But CI will catch it"_ → CI is a backstop, not a proof. Pushing
...
- _"But the previous commit was clean"_ → the current change is the
