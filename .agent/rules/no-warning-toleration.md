...
Operationalises castr's blocking-gate doctrine
([`DEFINITION_OF_DONE.md`](../directives/DEFINITION_OF_DONE.md),
[`quality-gate-failures.md`](quality-gate-failures.md)) at the warning
severity. Origin: Oak ADR-163 §6/§7 (entry-point boundary discipline +
non-deferrable-warnings amendment — cross-host).

Pattern reference: `acknowledged-warnings-deferred-to-the-stage-they-explode-in`
(Oak napkin 2026-04-23, first hard instance — cross-host origin).
...
- Treating warnings as "less serious than errors" for triage
  ...
  hierarchy is _root-cause depth_, not severity label.

## Required

- Every build script and every quality-gate command MUST treat
  warnings as fatal at the boundary it owns. Concrete examples in
  castr's chain:
  ...
  - madge / depcruise / knip: zero warnings — escalate to
    error-level in their configs.
  - Build tooling that reports warnings structurally (e.g. an
    esbuild-style `warnings` array): assert the count is zero in
    the build script.
- Any monitoring or observability surface the repo adopts MUST
  treat repeated warnings as a signal — alert on warning-level
  events and triage the `warning` severity bucket alongside
  `error`.
...
Recorded falsification (Oak origin, cross-host): 2026-04-23, deployment
...
its configuration declares — for formatting, the set of paths NOT
ignored by `.prettierignore`; for ESLint, the files the configured
flat-config covers; for depcruise/knip/madge, their config scopes; and
so on.
...
2. **Expanding the gate to cover a surface where the doctrine should
   ...
   (config-expert during planning, the gateway reviewer after fixes
   land).

Concretely for castr's markdown: `.prettierignore` is the canonical
record of the format gate's footprint. `.agent/` is deliberately NOT
ignored — new Practice docs are formatted by the gate automatically,
while excluding a reference-shaped folder requires an explicit ignore
line and the governance act that implies.
...
2. Escalate the warning to error in the same commit so the
   ...
   has been _converted to a blocking work-item with an owner
   and a deadline_.
...
- `config-expert` enforces the rule on every PR that touches
  gate configuration (ESLint, Prettier, vitest, tsconfig, husky,
  CI workflows).
