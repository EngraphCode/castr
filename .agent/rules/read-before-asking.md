...
The agent is about to ask the owner a question shaped like _what is
X_, _does X exist_, _what is the value of X_, _what does file Y
contain_, _what does the schema say_, _which records have field Z_ —
...
- preferences (_do you prefer A or B?_),
- approval (_should I do X?_),
- direction (_which path should we take?_),
- judgement calls (_is this trade-off acceptable?_),
...
The rule biases toward _investigation_ (reading is reversible). It
does **not** bias toward _execution_. An agent that reads an
...
If the agent finds itself reaching for _the path is named, applying_
or _the principles have settled this, acting_ — that vocabulary
...
- [`undo-change` skill](../skills/undo-change/SKILL-CANONICAL.md) — the same
  _render the diagnosis, then halt-ask_ shape applied to undo
  operations.
