...
(`.cursor/hooks/engraph-session-identity.mjs`; see
...
`.cursor/engraph-composer-session.local.json` mirror (when agent-tools is built)
...
`ENGRAPH_AGENT_IDENTITY_OVERRIDE` remains an explicit operator escape hatch for
...
(_"active means markdown-ritual, not code execution"_):

- **Session-close identity-update gate** — a documentation walkthrough
  in [`session-handoff` § Hard gate](../skills/session-handoff/SKILL-CANONICAL.md)
...
- **Stale-identity audit** — a six-check documentation walkthrough in
  [`consolidate-docs` § Thread-register freshness](../skills/consolidate-docs/SKILL-CANONICAL.md)
...
on all three is _"do not proceed until this step is complete"_ written in
