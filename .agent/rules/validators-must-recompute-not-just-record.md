...
A validator that _stores_ a derived value (content hash, file
fingerprint, manifest checksum, generated-output digest) but does
_not_ re-compute and compare on subsequent runs cannot detect
...
- The lock file _looks_ like it captures truth — it has hashes.
- The lock file _is_ not enforced — nothing reads the hashes back.
...
- Pure recording surfaces explicitly framed as audit logs (the
  ...
  _not_ validators; they are append-only history.
- Provenance fields (e.g. `provenance.yml` UUIDs) that record
  identity rather than content.

## Doctrinal Anchors

- principles.md §Strict And Complete Everywhere, All The Time (full
  enforcement, no hand-wavy partial systems)
- principles.md §Strict-By-Default and Fail-Fast (validators must
  fail, not silently accept)
