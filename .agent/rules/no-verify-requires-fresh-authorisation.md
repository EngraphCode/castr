...
Operationalises castr's blocking-gate doctrine ([`DEFINITION_OF_DONE.md`](../directives/DEFINITION_OF_DONE.md), [`quality-gate-failures.md`](quality-gate-failures.md)) at the hook surface: never disable quality gates, never disable Git hooks. The live PreToolUse policy (`.agent/hooks/policy.json`) denies the hook-bypass flag structurally.
...
skip. Owner authorisation binds to _this commit, with hooks skipped_ — the
