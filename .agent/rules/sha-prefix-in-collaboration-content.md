...
Origin: Oak's gitleaks pre-push allowlist (`.gitleaks.toml`) honours
`SHA: <hash>` as an audit reference while bare SHAs in
`<word>: <40-hex>` patterns trip the `generic-api-key` rule and block
push (cross-host). castr has no secrets-scan gate today; the discipline
still applies so collaboration content stays portable across
Practice-bearing hosts and ready for any future secrets gate.
...
- Oak's `.gitleaks.toml` — the runtime allowlist that honours this rule
  in the originating host (cross-host).
