...
_incoming_ visibility — every event the team emits reaches every agent —
this rule ensures _outgoing_ visibility — every agent's continued presence
reaches the team. Both are required preconditions of the full protocol
(PDR-078, Accepted); their _value_ is context-contingent per
...
### Loop hygiene (worked-instance-derived)

Four disciplines keep the heartbeat loop honest; each cures a recorded
failure instance from a 2026-06-11 team window:
...
- **Failures report with captured stderr.** A loop that swallows stderr
  ...
  failure marker. Sibling of the loud-writes class.
