...
**Impact**: 🟡 Medium (affects compatibility)  
**Effort**: 🟢 Low  
...
- ✅ Common: email, uuid, url, uri, date-time
- ⚠️ Others: Treated as string (no runtime validation)
- Workaround: Use `.refine()` for custom validation

### Discriminators

- ✅ Basic discriminator mapping
- ⚠️ Complex nested discriminators may need manual refinement
- Workaround: Use custom Zod refinements

### File Uploads

- ✅ Generates z.any() or z.instanceof(File)
- ⚠️ Requires manual handling in fetcher
...
⚠️  Experimental features enabled:
  • Multi-runtime support (unstable)
  • Async validation (beta)
...
- ✅ OpenAPI 3.0.x
- ⚠️ OpenAPI 3.1.x (mostly compatible)
- ❌ Swagger 2.0 (use converter first)
...
⚠️  Warnings (5):
  ...
  • Duplicate tag: "Users" and "users" (line 12, 67)
    Recommendation: Use consistent casing for tags

  • Missing description: POST /users (line 123)
    Recommendation: Add description for better docs

  • Large response: GET /users (>1MB)
    Recommendation: Consider pagination

  • No examples: Pet schema (line 234)
