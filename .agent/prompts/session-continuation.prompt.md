...
**Active operational entrypoint:** [Explicit Additional Properties Support](../plans/active/explicit-additional-properties-support.md)
...
The OAS 3.2 parent arc is now complete. Phase A2 closed on Friday, 10 April 2026. The MCP no-params follow-up, Phases B, C, D, and E, Husky local-workflow alignment, and generated-suite stability all closed on Saturday, 11 April 2026. Repo-root `pnpm check` is green on the landed explicit-`additionalProperties` implementation tree from Thursday, 16 April 2026, repo-root `pnpm check:ci` is green on the broad consolidation tree from Friday, 17 April 2026, and the ePerusteet real-spec validation slice remains the direct predecessor to the still-active explicit-additional-properties support plan. No successor atomic slice has been promoted yet, so that active plan remains the honest regression-first / successor-selection entrypoint.
...
- the broad durable-doc / staged-history consolidation closed on Friday, 17 April 2026: live doctrine, acceptance criteria, ADRs, staged completion records, and session-entry surfaces now align on the explicit-source-truth `additionalProperties` boundary, and repo-root `pnpm check:ci` is green on that consolidated tree
- no fresh product regression is currently reproduced, and no successor atomic slice has been identified yet; the active plan now remains only as the honest regression-first / successor-selection anchor rather than as an open semantic implementation slice
...
4. Treat the explicit-additional-properties slice as implemented and consolidated unless a fresh regression is reproduced.
...
7. Because no successor atomic slice is identified yet, keep the active plan as the honest regression-first / successor-selection entrypoint rather than promoting it prematurely to a completion record.
