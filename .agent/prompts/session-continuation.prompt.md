...
- the reviewer loop for the active explicit-`additionalProperties` slice is currently open on Thursday, 16 April 2026 with concrete findings: raw-surface OpenAPI tests need IR-preservation proof, ePerusteet needs an explicit first-pass OpenAPI round-trip/idempotence proof, `z.looseObject(...).catchall(...)` must remain rejected, decorated `z.any()` / `z.unknown()` catchalls must not collapse to bare `true`, first-pass ePerusteet round-tripping currently drops schema-level `deprecated: true` on nested schema-valued `additionalProperties`, and the Oak real-world fixture must be refreshed from the live upstream spec
- the later Thursday, 16 April 2026 reviewer rerun added four more concrete closure requirements: decorated permissive Zod catchalls must preserve first-pass `.default(...)` on write-back, recursive explicit-`additionalProperties` Zod output must fail fast in nested property / array-item / composition-member contexts too, raw-surface OpenAPI proofs must cover schema-valued `additionalProperties` on all claimed carriers, and the generated ePerusteet rejection proof must assert the narrowed explicit-`additionalProperties` capability boundary rather than a broad alternation
- the subsequent Thursday, 16 April 2026 reviewer follow-up added three more proof requirements: the recursive catchall guard needs explicit `prefixItems` regression tests, property-helper getter detection needs a tuple-only recursion proof, and the refreshed Oak real-world fixture must have a transform-round-trip proof rather than load-only coverage
- the same follow-up also noted a reproducibility requirement: the refreshed Oak real-world fixture must be carried as a committed repo fixture, not left local-only, or clean-checkout coverage will drift immediately
- the later type-review follow-up added two stricter closure requirements: the recursive Zod catchall fail-fast guard must reject direct `$ref` catchalls and catchall-object recursion that is reachable only through ordinary properties, and OpenAPI component circular-reference detection must include `prefixItems` so tuple-only recursive schemas receive the same metadata markers as property/item cycles
...
6. Treat `deprecated` as an OpenAPI schema field that must be preserved honestly once confirmed by the active evidence set; do not permit first-pass metadata loss to hide behind later idempotence.
7. Use [eperusteet-real-spec-validation.md](../plans/current/complete/eperusteet-real-spec-validation.md), [oas-3.2-full-feature-support.md](../plans/current/complete/oas-3.2-full-feature-support.md), and [phase-a2-type-migration.md](../plans/current/complete/phase-a2-type-migration.md) only for predecessor context.
