...
| Proof-programme Routine (scheduled trigger: stored prompt, cadence, model, notifications, sources)                                       | The autonomous background loop (ADR-051) firing at all                                                                 | [Parent plan §Operating protocol step 1](../plans/proof-programme/parent-plan.md): create a scheduled task with this repo attached, stored prompt a thin pointer to `routine-prompt.md` on main, cron `3 */8 * * *`, notifications per [`account-access.md`](./account-access.md), and the per-trigger "Auto-fix pull requests" setting OFF (firings drive their own PRs; a platform-side second writer breaks the lease/head-recheck model) | The first scheduled firing observed end to end: credentialed landing + completion notification received  |
