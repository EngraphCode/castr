...
This directive is grown by the multi-agent-collaboration-protocol (an
Oak-origin plan; castr has no local equivalent plan yet). It
...
- _do not proceed until you have consulted the surface and decided how to
  coordinate_ — substance of the decision is agent judgement;
- _do not break the build unless you will fix it soon_ — substance of "soon"
  is agent judgement.
...
The coordinator role becomes the _expected_ default when a super-
...
The role is a _commitment to coordinate_, not a new primitive. Any
...
The non-negotiable invariant — castr's `principles.md` §Strict And Complete
and the `dont-break-build-without-fix-plan` rule — is:
...
rule fires as a tripwire: _do not proceed until you have consulted the
surface and decided how to coordinate_. It does **not** fire as: _refuse
if a claim exists_. The companion
...
shared log, log _"no other agents present"_, register the session claim,
...
PR closeout names _when_ reviewer-comment state must be harvested;
PDR-015 names _whose_ review authority applies on which abstraction
layer.
