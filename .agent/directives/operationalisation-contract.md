...
contract of where content lives) by adding the _flow contract_ of how
...
| **3. Worked Examples + Recipes**     | Per-instance files indexed at the parent mechanism (e.g. patterns library entries, recipe pages in governance docs)                     | The _what-it-looks-like_ — small focused per-instance files.                            |
...
| Sub-agents / specialist reviewers | `.agent/sub-agents/` (+ thin platform adapters) | Review-time (adaptive immunity) |
| ESLint custom plugins             | `lib/eslint-rules/` (+ `lib/eslint.config.ts`)  | Lint-time                       |
| Quality gates                     | `pnpm check`, `pnpm test`, etc.                 | Pre-commit + CI                 |
| Skills                            | `.agent/skills/*`                               | Workflow-time                   |
| ADRs                              | `docs/architectural_decision_records/`          | Architectural-decision record   |
| PDRs                              | `.agent/practice-core/decision-records/`        | Practice-decision record        |
| Governance docs                   | `docs/architecture/*.md`                        | Detailed elaboration            |
...
`consolidate-docs` audits this alignment at consolidation time.
...
ephemeral pointer. The forbidden direction is _permanent → ephemeral_
only.
