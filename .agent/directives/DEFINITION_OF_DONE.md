...
- **Fidelity suite**: edge-case fixtures (`lib/tests-transforms/__fixtures__/edge-cases/`) prove boundary-revalidated IR round-trip (written output reloaded through `loadOpenApiDocument`, so spec-invalid writer output is red), serialization round-trip, byte-stable rewrite, and — for parser-lossy traits — source-level `sourceAssertions` per fixture via the fidelity harness inside `pnpm test:transforms` (see `docs/architecture/fidelity-proof-harness.md`)
