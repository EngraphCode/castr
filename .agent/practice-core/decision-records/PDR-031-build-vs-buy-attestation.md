...
explicitly: _"Searched npm registry for `<terms>`; reviewed the
top-N matches; none provided `<required-property>`. Searched the
internal codebase for `<terms>`; no existing implementation."_
...
_and the unreliability is invisible at plan-write time_. A soft
