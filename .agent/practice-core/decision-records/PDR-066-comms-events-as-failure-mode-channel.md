...
_capture_ stage's real-time vehicle for failure-mode substance);
...
2. **Session close is the wrong cadence.** Failure modes
   ...
   cast, the _next_ coordinator may already be running, paying
...
them visible to the next coordinator's watcher _during the next
coordinator's session_, not at the prior coordinator's close.
...
consolidation surface becomes the _consolidation_ destination for
...
### What does _not_ get surfaced in real time
...
- **`behaviour-note`** — softer category: a noticed behavioural
  ...
  agent's attention (e.g. _"the X reviewer is currently returning
  Y verdicts when Z is in scope; reshape briefs accordingly"_).

The discriminator composes with the existing channel
discriminator (broadcast / group / directed / lifecycle): a
failure-mode broadcast is _broadcast + failure-mode_; a directed
behaviour-note is _directed + behaviour-note_. The substrate
...
- Additional comms-event volume during team sessions. Each
  ...
  subscription.
- A new substance-judgement cost on every agent: _"is this
  observation substantive enough to surface, or is it routine?"_.
...
2. **Cross-session failure-mode threading.** When a failure mode
   ...
   substance is _the same failure recurring_; stand alone when
   the substance is a _new failure with similar shape_.)
