...
- **2026-08-24 — cloud seats seed from the platform session id; hooks never
  ...
  The measured failure (2026-08-24, castr — one seat, three identity tuples
  in one day) is a pinned name surviving a seed change, yielding a
...
- **2026-05-29 — retired/completed thread records carry a retirement
  ...
  This extends the same hygiene to the _thread-record_ level: when a thread
...
- **2026-04-21 Session 5 — workstream layer retired as an
  ...
  thread).** The Decision body says _"one thread may span
  multiple workstreams"_ and _"workstream briefs are scoped to
  threads"_ — this framing remains valid as abstract vocabulary,
  ...
  **Semantic position**: threads still map 1:many to workstreams
  _in principle_ (the body retains this); the change is
...
neither PDR names explicitly is the unit that _persists across
sessions_. A session is the wrong unit for continuity — it is
...
3. **Identity drift across sessions.** Without a schema for
   _who_ participated, a resuming agent cannot tell whether it is
...
**A _thread_ is the continuity unit.** A thread is a named stream
of work that persists across sessions and may be touched by
multiple agents over time. A _session_ is a time-bounded agent
...
- A workstream or lane — workstreams describe _scope_ (what is
  being built); threads describe _continuity_ (what stream of work
...
- **"Lane"** — short, but connotes parallel tracks _within_ a
  workstream, not the continuity unit _above_ the session.
...
A session is by construction time-bounded. Continuity _must_ be a
...
the _workstream_ — but workstreams are scope-bearing, not
...
_descriptive_ — they annotate the identity but do not participate
