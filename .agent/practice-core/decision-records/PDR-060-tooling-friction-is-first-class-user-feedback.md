...
_users_ reporting friction to _authors_ through external channels —
...
session): _"any friction with agent tooling should always be noted
...
backlog rather than left only in chat."_
...
3. **Chat is not a capture surface.** A friction observation that
   ...
   distillation can happen later. The discipline is _that_ capture
...
**Why it is not an ADR.** The doctrine is not about _this repo's
product architecture_. It is about _how the Practice operates_ — the
...
work _downstream_ of this PDR, not a substitute for it.
...
- Sessions that _consume_ agent-tooling actively look for friction to
...
- Capture overhead inside sessions that hit friction. The cost is
  one to three lines per observation; the cost of _not_ capturing is
