...
checks reasoning _inside the plan's worldview_.
...
_Empirical instance (2026-04-26 morning, Sentry vendor-doc review;
host-local identifiers omitted from this portable record)_: a
...
_violates the vendor's documented contract_. Internal reviewers
...
_Empirical instance (2026-04-26 afternoon, Sentry fingerprinting;
host-local identifiers omitted from this portable record)_: an
...
_"What capabilities does this vendor offer that this plan does not
reference?"_
...
- The decline list captures _why_ a capability was not used, so
...
- The two empirical instances both occurred on the same vendor
  ...
  argument is that the _failure mode_ is platform-agnostic even
