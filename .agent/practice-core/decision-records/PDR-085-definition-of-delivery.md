...
the first application exercised the doctrine on a gate-staged delivery plan)
**Related**:
...
2. **The unusable response.** A surface emits more than its _immediate_
...
**producer** question — _did I finish my task?_ "Delivered" is a
**beneficiary** question — _did someone receive value they can use?_ Without a
...
2. **Real value, integrity intact** — the beneficiary can do something they
   ...
   Correctness, preserved caveats, and being _within the consumer's budget_
...
5. **Observable signal of receipt** — falsifiable evidence the beneficiary
   _can_ receive the value (a completable journey, a test on the real path,
   an exercised importable surface, telemetry). The boundary: _can-receive_
   evidence, not _did-receive-at-scale_ proof.
...
discovery harness, a metric — delivers to a real beneficiary class: _the team
enabled to explore_. The six criteria apply unchanged with that beneficiary
...
inside. Tying the definition to _receipt by a named beneficiary_ means work
...
- **Relationship to PDR-026.** PDR-026 governs the _session_ unit (what a
  session commits to land and reports at close); this PDR generalises the
  same insight to the _beneficiary-received_ unit, which recurs below and
