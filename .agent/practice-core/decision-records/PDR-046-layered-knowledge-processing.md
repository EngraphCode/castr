...
_sequenced_ deferral pattern at the layer boundary, not a hidden
...
The diagnostics are honest reports about the _shape_ of each
...
_individual capture or graduation_: **learning preservation
...
The per-write rule is sufficient when work happens _one write at a
time_. It is insufficient when work intentionally traverses
...
below. The fitness signals on the _upper_ layers fire during
...
substance and notices a fitness signal _on Layer N itself_
...
not name the obligation to _keep walking_ up the staircase once
...
layer have NO jurisdiction over the agent's actions while _that
layer or any lower layer_ is being processed. Fitness becomes
...
A fitness signal on Layer N+1 _at the time Layer N processing
begins_ is irrelevant to Layer N's work. The signal will be
...
processing turn, fitness diagnostics on _that_ layer are also
...
3. After the pass on this layer completes (i.e. all source
   ...
   layer is at rest. _Then_ the fitness signal becomes
...
exists to keep the _substance_ uncompressed; mid-pass form-keeping
...
default response is _not_ to compress the layer's existing
...
pressure) is correct but incomplete. It governs the _moment of
individual capture or graduation_ and forbids in-the-moment
...
not say what to do when the _destination layer's_ fitness signal
fires during processing of a _different_ layer. Without an
...
- The discipline requires the agent to recognise _which layer_
...
- The rule does not constrain _how many_ layers a Practice has.
...
- The rule does not require that every pass touch every layer.
  ...
  needs processing). The traversal is bottom-up _over the
  substance present in this pass_, not over every layer
  unconditionally.
...
_sequenced_ deferral, not a hidden declaration of non-action), and
