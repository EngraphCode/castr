...
- **2026-04-21 Session 5 — consolidating rewrite (Pippin /
  ...
  1. The two 2026-04-21 Session-4 amendments ("_active means
     markdown-ritual, not code execution_" and "_Class-A.1
     Layer-2 retraction: foundation-directive grounding is the
     implicit Layer 2_") are **absorbed into the Decision
  ...
  2. **Class A.1 Layer-2 reclassification: foundation-directive
     ...
     directive grounding is _ordinary session-open grounding_
...
unless something in the environment _fires_ on a named condition
...
additive-identity rule is _passive guidance_ (a convention
...
Finally, a _meta-level_ failure mode threatens any durable
...
Heath in _Decisive_ ch. 9 and _Switch_ ch. 8). The load-bearing
...
comes from the ritual's _"do not proceed until X"_ obligation,
...
_"decide X"_ or _"consult and judge"_ without a specified
...
- **Ritual-form tripwires** (gates, walkthroughs in commands)
  ...
  already encodes this with _"MUST NOT rely on self-reporting"_.
...
says _"do not enter if a claim exists"_ or _"halt if X is
true"_ — invites routing-around at the cost of architectural
...
- **Family A** rules fire as _"do not proceed until you have
  consulted the surface and decided"_, not _"refuse to
  proceed if condition Y is true"_. The substance of the
...
foundation grounding is _ordinary session-open grounding
required by PDR-013 regardless of this PDR_, not a distinct
...
1. **Per-consolidation meta-check** — at every consolidation
   ...
   _"Did any content in this pass resist classification into
   one of the taxonomy's modes? Did any content fit multiple
   modes ambiguously?"_ Resistance and ambiguity are signals
...
ritual's _"do not proceed until X"_ obligation, which every
...
uncovered platforms carry artefact gravity _and_ lack the
...
Family A protects against failure modes _within_ a memory
...
_persistent mis-organisation_ of the repo's memory that silently
...
- **Single-layer tripwires per class _as a general rule_.**
...
- **Automated perturbation (e.g. random re-ratification).**
  Rejected — the design priority is firing _at the right
  moment_ (shape-entry, thread-join, session-close), not
