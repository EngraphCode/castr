...
A _cure_ that addresses a recurring failure mode can be authored at any of
three layers:
...
3. **Asymmetric** — a behavioural cure that is structurally enforced _only
   on the operators who follow the discipline_. The cure protects the
...
Asymmetric cures are _visible only after the failure mode has fired_.
...
The asymmetry insight (owner-named): _a cure that protects the agent
...
one side keeps on the other side's behalf._

## Decision

When authoring or reviewing a cure for a recurring failure mode in
shared-state work, classify the cure's _symmetry_ explicitly. Three
shapes:
...
2. **Asymmetric structural cure**: the cure fires only when _this_
...
discipline for _shared_ failure modes.

## Rationale

The asymmetric-cure failure mode is:

- _Invisible until it fires_. The applier completes the work correctly;
  the failure mode shows up only when a peer triggers the same shape
  while the applier is mid-flight.
- _Compounding under multi-agent load_. As N grows, the probability that
...
- _Reframed as "the absent agent's responsibility"_. Authoring a
...
trades belong in a host ADR; the _requirement_ to install a symmetric
...
`pending-graduations.md` entry _"PDR-shaped follow-up (Practice-
governance principle): asymmetric-cure failure mode — a cure that
protects only the applier is not really a structural cure"_ (captured
