...
- Briny Lapping Harbor's _"Option B: disable canonical default"_
...
_when principles, a reviewer, or prior owner-reframe has already
named the path, apply rather than ask_. The candidate was quarantined
...
doctrine's general bias — _acting beats asking when the path looks
named_ — shaped the action landscape; the rule, as written,
contained no destructive-operation guard.

The owner reframed at the same close: the load-bearing distinction
is not action-vs-asking. It is _empirical answerability_ — whether
...
2. **The discharge action is reading, not acting**. The gate
   ...
   _"therefore execute the destructive operation"_.
...
- Treating _"I assumed it was here"_ as equivalent to _"I read it
  and confirmed it is not here"_.
...
_what gets read into the working state_; PDR-057 disciplines _what
gets asked of the owner_. Neither subsumes the other.

The empirical-answerability frame carries no inherent action-bias.
The gate's _"if yes, read"_ discharge is non-destructive by
...
Apply-don't-ask conflated two distinct shapes: _the agent is bouncing
the answer to the owner instead of reading_ (a discipline failure)
and _the agent is hesitating to act on a clear path_ (which, as the
...
- The agent reaches the owner with questions whose empirical
  ...
  _"please look at the file I have not read"_.
...
- Sessions that previously raced to the owner with _"please
  decide"_ rounds now pay an empirical-surface read cost first.
...
- Drafting a substantive question to the owner before any
  ...
  triage — _"is this even worth investigating now?"_ — is a
  legitimate value-judgement question and falls outside the gate.)
- Treating _"the answer is probably in the code somewhere"_ as a
  substitute for reading.
- Reframing the rule as _"act, do not ask"_. The rule does not
...
prompt that asks _"which empirical surfaces did you read?"_ before
