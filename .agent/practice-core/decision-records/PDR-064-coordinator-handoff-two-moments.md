...
_"2026-05-21 — Coordinator-handoff: pre-positioning vs active-
acknowledgement"_ (the entry carries the specific session
...
- Owner correction: _"you are coordinator until \<incoming\>
  actively acknowledges taking over the role."_
...
kind with a clear title (e.g. _"Coordinator pre-positioning:
\<outgoing\> → \<incoming\>"_). No schema change is required.
...
- explicitly states _"I am taking the [slice] coordinator role
  from \<prior\>"_ in the title or first line of the body (the
...
_inside_ the full-coordinator window, not in place of it.
...
- **Moment 1 (pre-positioning) for a slice transfer** is a narrative
  ...
  _"slice-coordinator pre-positioning for [slice boundary]:
  \<outgoing\> → \<incoming\>"_ — so peers know the transfer is
...
- **Moment 2 (active-acknowledgement) for a slice transfer** is a
  ...
  conventional title _"Slice-coordinator role acknowledgement
  for [slice boundary]: \<incoming\> from \<prior\>"_, referencing
...
precisely this distinction: _"you are coordinator until \<other\>
actively acknowledges."_
...
coordinator-role artefact. The rule _"cadence ends at Moment 2"_
...
- Pre-positioning events use clear, conventional titles
  (_"Coordinator pre-positioning: \<outgoing\> → \<incoming\>"_)
  so watchers can identify them at a glance.
- Active-acknowledgement events use clear, conventional titles
  (_"Coordinator role acknowledgement: \<incoming\> from
  \<prior\>"_) and reference the pre-positioning event via
  `in_response_to`.
- Owner-mediated coordinator selection (the owner naming the
  ...
  before authority transfers. Owner direction is final on _who_
  the coordinator should be; this PDR governs _when_ authority
  transfers.
...
- A possible coordinator-less _waiting_ state if active-
...
4. **Owner-direct designation without pre-positioning.** Owner
   ...
   broadcast that explicitly names _"I am coordinator from
   session-open per owner direction"_ satisfies Moment 2 by
