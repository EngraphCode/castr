...
- **2026-05-09 amendment — five planning-discipline rules graduated
  ...
  - **Dry-run multi-step workflows against accumulated state**
    before committing to the recipe. The dry-run produces a _proceed_
    or _stage differently_ signal; treating multi-step workflows as
...
- **2026-05-04 amendment — beneficial prerequisites must not block
  ...
  _blocking_ prerequisites (the dependent work cannot exist or
  function without them) from _beneficial_ prerequisites (the
...
- **2026-05-14 amendment — DECISION-COMPLETE is the readiness gate
  ...
  applied after execution. Every execution-time decision that _can_
  be settled at plan-author time _must_ be settled there. The
...
3. **Place at lane × actionability**, regardless of how many
   ...
   feels crowded, the response is _split the collection_ or
   _archive completed plans_, not _redirect the new plan to a
   thinner directory_.
...
2. For each `beneficial` prerequisite, the plan states the dependent
   work's _minimum shippable shape without it_. If the plan cannot
...
3. The default cure for a beneficial-but-coupled-as-blocking shape
   is _additive_: ship the new capability alongside the existing
   surface; defer the migration of the existing surface to a later
   plan. The integrated rename / migration / consolidation is the
   _optimisation_, not the _prerequisite_.

**At plan-estate audit cadence (consolidation pass):**

A consolidation step asks: _what plan is currently gated behind
which prerequisite, and is the gate real?_ Each gated plan answers
...
implementation plan, every execution-time decision that _can_ be
settled at plan-author time _must_ be settled there: vendor literals,
...
_is_ the unresolved decision: resolve it before promoting the plan,
