...
1. **Atomic judgment, deterministic aggregation.** An LLM stage emits _only_ local, per-item,
   ...
   _reporting_ carries the same duty: when matches or verdicts have unequal evidence strength,
...
2. **Conserve by default; corroborate the irreversible disposition.** No high-stakes irreversible
   ...
   **diverse-lens quorum** — votes from distinct lenses so they are uncorrelated, which is what
   ...
   `principles.md` §Agentic Quality risk-tiering applied correctly: _highest rigour where harm is
   asymmetric and irreversible_. A terminal discard is neither cheap nor self-correcting, so it
...
4. **Cost and throughput are deterministic, re-gated, and orthogonal to rigour.** Pre-spend cost
   ...
   2026-07-02) — least-privilege agent types are an _economic_ primitive as much as a safety one.
...
- **This pipeline is a discovery FEEDER, not a conservation engine.** Its validated findings flow
  ...
  the _feeder_ mechanisms vary (corpus analysis, session learnings, curator passes), but the
  _conservation outcome and machinery are shared_. Do not build a bespoke graduation step for any
