...
_before_ exhausting the inventory of artefacts and capabilities
...
they were _at the time the brief was written_. By the time the
...
output, the next move is _not_ to retry the same tool with adjusted
parameters. It is:
...
a constant-type predicate), the next move is _not_ to author the
...
blockers, the next move is _not_ to act on the enumeration as
written. It is:
...
- The three moves together generalise into the broader principle —
  _internal cheap fresh source before external expensive stale
  source_ — which extends to PDR-033 (vendor-doc review) and to
