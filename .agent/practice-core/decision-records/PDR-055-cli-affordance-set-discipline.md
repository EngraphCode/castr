...
4. **Full-help on flag failure**. Every CLI invocation with an
   ...
   the _full_ help text (the same output as `--help`) before exiting
...
6. **Use built artefacts, not source-on-each-invocation**. CLIs are
   invoked via their _built_ dist artefact, not rebuilt from source
...
The coordination affordance set is load-bearing precisely _because_ it
...
_"all invocations of ALL agent tools with improper flags MUST print
the FULL help text"_. The built-CLI rule is owner-stated standing
direction: _"all agents use only the built agent tools, so that
...
PDR's substance is the _requirement_ shape, not the implementation
sequence.
...
`pending-graduations.md` entry _"PDR/ADR candidate — agent-tools CLI
affordance set + build isolation"_ (captured 2026-05-05 from the 7-agent
