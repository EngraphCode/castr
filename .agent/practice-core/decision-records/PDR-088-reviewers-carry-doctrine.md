...
_be_ before its findings carry weight);
...
it is a _structural auditor_: it runs output against a checklist of
surface properties and reports mismatches. In the second, it is a
_carrier of the doctrine it enforces_: it has internalised the governing
principles and reasons from them, so its findings name _why_ a thing is
wrong in the doctrine's own terms, not merely _that_ a checklist item
failed.
...
the auditor still produces output that _looks_ like review. The cure is to
make the reviewer carry the doctrine — and to make that carrying a
_forcing function_ in the reviewer's own definition, not an aspiration.
...
documents _on every invocation_, with lazy loading explicitly forbidden.
...
non-internalisation _observable in the output_ — a reviewer that has not
