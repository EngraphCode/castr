...
cleanly answer the question _"which kind of record is this?"_.
...
_"could this record land unchanged in another repository adopting
the same practice?"_. If yes, it belongs in a PDR. If the substance
...
- Branch-name conventions
  (the conventional-commits-style prefixes that name _this_ repo's
...
- Cross-references to portable practice surfaces inside the
  practice-core area, using _relative_ paths internal to the
...
- _Yes, unchanged_ — it is a PDR. Authoring should pass each of the
  portability constraints listed above.
- _No, it names this repository's substrate_ — it is an ADR.
...
discipline is scope-updated to apply strictly to _portable surfaces_
(PDR files; rule files in the rules directory; pattern files in the
memory/patterns directory) and explicitly NOT to _repo-bound
surfaces_ (ADR files). The rule cites this PDR as the governing
...
already in place, but the rule's _scope_ erroneously named ADRs
...
- An ADR that softens its repo-bound substance into portable
  vocabulary to _appear_ PDR-like.
