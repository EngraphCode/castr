...
- **Two epistemic axes, never conflated**: _computed confidence_
  (usage counts, distinct documents, distinct strata — always derived
  from the graph, never authored) and _authored status_ (the lifecycle
...
Authored current-state is mutable _with history_, at two grains kept
