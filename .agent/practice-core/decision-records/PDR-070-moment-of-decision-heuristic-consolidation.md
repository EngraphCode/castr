...
the _enforce_ stage's organising surface);
...
rules by _when they should fire_. The moment of decision is
the densest locus: many rules converge there. A single
decision-time heuristic — _at every decision point, the
question is which shape gives long-term architectural
excellence_ — is a candidate to subsume many rules' verdicts
at that locus.
...
moment of decision. The five rules carried the _reasoning content_
...
> _Which shape gives long-term architectural excellence?_

The rules continue to carry the reasoning content — the
_why_ behind each shape's excellence-relative-to-alternatives —
...
- How does the heuristic compose with rules whose subject is
  _meta_ (capture discipline, observability, identity) rather
  than _architectural_?
