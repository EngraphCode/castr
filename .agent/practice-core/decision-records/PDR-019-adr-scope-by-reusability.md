...
- **Code with TSDoc** — the canonical home for _how_ a decision
  is realised in a specific module, with intent and trade-off
  context inline.
- **Plan bodies** — the canonical home for _how_ the decision is
  rolled out across the codebase (sequencing, gates, rollback).
- **Runbooks and READMEs** — the canonical home for _how_ to
