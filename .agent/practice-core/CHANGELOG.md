...
## [castr] 2026-09-20 — PDR-049 merge-driver registration shape (Candle weaves Residue)

- **Amended**: `decision-records/PDR-049-memory-and-state-file-merge-semantics.md` §Investment Staircase option 3 — a merge driver is registered as a path relative to the checkout top level, so one registration arms every linked worktree and each checkout runs its own build; bare-repository and server-side merges never invoke a driver. The cost line now names a per-repository bootstrap plus a per-checkout build. Added an Amendment Log.
- **Amended**: `decision-records/README.md` index — PDR-049 status.
...
- Adopted Oak's advanced trinity and entry points: the six-audience knowledge-flow model, the prompts→skills migration, and the patterns-taxonomy / PDR-007 Core contract
- Migrated provenance from the inline `provenance:` array + `fitness_ceiling` to the `provenance.yml` pointer + multi-dimensional fitness model (`fitness_line_target`, `fitness_line_limit`, `fitness_char_limit`, `fitness_line_length`)
- Created `provenance.yml` as the union of both branches' per-file chains — preserving castr's 2026-03-22 branch-local entry (which Oak's chains lacked) and adding a 2026-06-05 merge node per tracked file
- Retired the `.agent/practice-context/` surface per the PDR-007 amendment (2026-04-29); archived castr's authored `outgoing/` notes and repointed the live navigational references
