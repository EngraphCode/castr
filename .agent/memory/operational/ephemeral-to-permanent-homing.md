...
| ----------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Host-repo architectural decisions                           | ADRs in `docs/architectural_decision_records/`                                                                                                  |
...
| Operator runbooks, deployment context, on-call material     | `docs/operations/*.md` (host-local operations docs; castr is a headless library — this home is dormant until such content arises)               |
...
enough to keep?"** Per the user's standing direction: _all content must
be moved to permanent homes or, if not useful, removed_. Removal is a
valid outcome.
...
- **Do NOT delete drift content without homing it first** unless it is a
  ...
  another permanent home. The user's standing direction is _find proper
  homes first_.
...
phrasings — _"budget consumed"_, _"out of scope"_, _"for later"_,
_"next session"_, _"ran out of time"_ — are not acceptable.
