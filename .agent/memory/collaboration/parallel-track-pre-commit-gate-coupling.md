...
graduated_from: imported (Oak Practice collaboration-patterns class; founding entry)
instances: 3
...
The founding entry of the collaboration-patterns memory class — the pattern
that, in the Oak Practice estate, motivated the multi-agent collaboration
protocol now expressed in castr by the
...
This pattern is the motivating evidence for castr's agent-to-agent
collaboration protocol, which installs:
...
castr's collaboration-state engine (structured claims, decision threads, the
commit-window) extends the structural fix toward higher-fidelity coordination
state. The pattern's complete enforcement edge lands when accumulated
multi-session evidence either confirms the advisory model is sufficient or
triggers named hook-based reminders.
...
Future instances will validate or refute the advisory model; the next
refinement amendment cites real evidence from this register plus napkin
entries.
