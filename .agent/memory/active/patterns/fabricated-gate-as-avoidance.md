...
> avoid_, not a shape to repeat. The "pattern" is the agent's
...
entry. The gate language is heuristic — _"this looks XL"_, _"second
instance not yet observed"_, _"plan execution will validate"_ — and
...
3. The gate vocabulary has _replaced_ the substance assessment —
   the next reader sees `size: XL` and stops at the size, never
   reaching the substance question _"is this graduation-ready?"_.
...
- The **30%-context-budget for directive-file processing** entry
  (DUE-1) explicitly said _"PDR landing is itself directive-shaped
  work that requires <30% context, which this session does not
  have"_. But the entry's substance is a Practice-Core PDR — not a
...
- The **orchestrator-vs-gate cure** (DUE-2) said _"combined
  directive + script + skill scope exceeds this drain session's
  context budget; sequenced for next slot"_. Then was deferred
...
- **Vaporware-gated** entries (DUE-5, DUE-6, DUE-7) gated graduation
  ...
  for the doctrine — the plans were _applications_ of the
  doctrine, not its source.

Owner correction:

> _"this IS the session where we graduate the pending graduates,
...
1. **Read the substance, not the size tag.** Can a competent agent
   ...
   a verdict.
2. **Name the gate's actual constraint.** _"vaporware-gated on
   WS11.3 execution"_ — does WS11.3 execution affect the doctrine's
...
3. **Distinguish epistemic gates from capacity gates.** _"N≥3
   validation required"_ is an epistemic gate when it preserves the
   ...
   2026-05-07 _"sequenced for next slot"_ gate accumulated >5
...
4. **Notice when the gate's vocabulary tracks the deferring agent's
   ...
   tagged `XL` with `sequenced-deferral pointer` written _during_
...
- [`templates-encode-failure-modes`](templates-encode-failure-modes.md):
  ...
  schema fields are useful — but the _vocabulary_ in the values
...
- [`mechanical-sequence-is-activity-bias-diagnostic`](mechanical-sequence-is-activity-bias-diagnostic.md):
  ...
  _feels_ like progress (each entry receives a verdict) while the
...
2. Ask the substance question: _"is the doctrine / cure / decision
   captured here graduation-ready?"_ Answer in terms of substance
   completeness, not gate vocabulary.
3. If yes, graduate it. Mark the entry `status: graduated`. Do not
   re-classify under the gate.
4. If no, name the _missing substance_ (not the missing gate
...
5. After processing, audit any new entries this session adds. Any
   ...
   must name a _real_ constraint that survives the substance test
