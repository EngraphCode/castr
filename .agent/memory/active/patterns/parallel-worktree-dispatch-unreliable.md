...
Concurrent subagents in one batch can resolve to _different_ base
...
2. **Worktree boundary is not enforced for absolute-path writes.**
   Worktree isolation is filesystem-isolated for _file creation in
   the worktree path_ but does not block absolute-path Edit/Write
