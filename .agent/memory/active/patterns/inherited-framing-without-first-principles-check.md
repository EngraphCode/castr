...
1. **Shape clause.** Is the test-shape (or implementation-shape,
   or doctrine-shape) right for the **first-party behaviour**
...
3. **Instance 3 — probe assertion-kind translated forward
   ...
   preserved the _kind_ of assertion the old code made ("these
...
4. **Instance 4 — §L-8 WS1 three-test spec asserted vendor /
   configuration behaviour, not first-party behaviour.** The
   plan prescribed three integration tests at the build-config
   boundary; the first-party logic worth testing was a single
   env-to-plugin-config translator. The three-test shape
   proved "the vendor did its job" not "the first-party translator is
