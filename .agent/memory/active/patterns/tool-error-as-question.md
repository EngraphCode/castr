...
polarity: pattern
use_this_when: 'a tool, signal, hook, or reviewer returns a non-pass result and the impulse is to find a way past it rather than understand what it is asking'
category: process
status: provisional
...
| ------------------------------------------ | -------------------------------- | --------------------------------------------------------------------- |
| Reviewer (sub-agent)                       | Reviewer surfaces a finding      | Treat the finding as the _only_ thing wrong; let it define plan scope |
