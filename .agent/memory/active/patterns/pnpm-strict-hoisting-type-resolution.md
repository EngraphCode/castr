---
name: 'pnpm Strict Hoisting Blocks Transitive Type Resolution'
polarity: anti-pattern
use_this_when: 'a type-check fails on an import of types from a package that is only a transitive dependency under pnpm strict hoisting'
...
> **POLARITY: ANTI-PATTERN.** This entry names a _failure mode to avoid_, not a shape to repeat. The name is the diagnostic: when type resolution silently fails on a transitive dependency, recognising the shape is the first move in not repeating it.
>
