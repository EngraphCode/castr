...
- **One repeating structural shape** — every observation is an
  ...
  load-bearing solution shape. The right output is a _contract_:
...
- **N independent findings** — each observation is its own
  ...
  output is a _report_ that routes each finding to its
...
> _Are these findings instances of one shape, or are they N
> separate shapes that happened to surface in the same pass?_
...
accumulated by _adjacency to existing sections_ rather than
_first-principles match to substance-kind_: apparently-orphaned
...
gates _all_ shared-state mutation, not just claim opening.

The findings were all instances of one shape: _substance-placement
followed adjacency-of-prior-edit, not substance-kind_. The right
output was therefore the contract
`collaboration-state-placement-contract.md`
...
> _Can I write the shape these findings share in one sentence
> that names a load-bearing structural property?_
