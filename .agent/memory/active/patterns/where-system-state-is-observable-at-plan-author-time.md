...
polarity: pattern
use_this_when: 'planning a multi-cycle structural change and deciding at which boundary each cycle’s tests should describe the system state'
category: process
status: provisional
...
> **POLARITY: PATTERN.** This entry names a _shape to repeat at plan-author time_: a single diagnostic question that constrains test-shape decisions across every cycle in a multi-cycle plan.
...
tests should describe _that_ boundary. Cycles that produce tests at
...
> _Where will the system state this plan is moving toward be
> observable from a test, in the form it actually runs?_
...
implementation details" — but the anti-pattern fires _at plan-author
time_, before any test is written. Asking the describing-surface
...
When reviewing a plan's cycle decomposition, ask: _would each
cycle's tests survive a refactor that changes the internal seam
without changing the observable workflow state?_ If no, the tests
