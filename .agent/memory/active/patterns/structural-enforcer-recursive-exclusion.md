...
A second anti-pattern shape: an exclusion list that names _some_ but
not _all_ cataloguing documents, so the enforcer fires inconsistently
across the cataloguing surface. The list must be exhaustive over the
documents that exist _at the time the enforcer is authored_; new
...
pattern's exclusion list is _not_: a clause inside the enforcer
...
- A hedge is **inside** the rule specification: _"the trip-list catches
  vocabulary X, **except** when the document is teaching about
  vocabulary X."_ The substance test asks whether re-phrasing reads "the
...
- A recursive-exclusion list is **outside** the rule specification: the
  ...
  — they are meta-doctrine surfaces _about_ doctrine. The substance
...
trip-list catches: _carve out_, _exception_, _for these arcs_, etc.
...
thirteen scoped*blocks. No hedge was added to PDR-047 itself; the rule
remains absolute. The exclusion list reflects that PDR-047 is a
meta-doctrine surface (it is \_about* the trip-list), not a doctrine
...
vocabulary; Ferny's case was vocabulary literals against a _new_ PDR
...
is wired against _paths_, and the documents that catalogue the
...
- **Governance Claim Needs a Scanner** (`governance-claim-needs-a-scanner.md`)
  — paired pattern. That pattern names _when_ a scanner is required
  (universal claim across live surfaces); this pattern names _how_ the
...
- **Passive Guidance Loses to Artefact Gravity**
  ...
  active enforcer wins, _but_ an active enforcer without recursive
