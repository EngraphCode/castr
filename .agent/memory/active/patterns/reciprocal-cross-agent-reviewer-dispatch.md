...
- SVW → Sparking on `745fe919` (t13a freshness check): TSDoc filename
  ...
  `freshness-binding.unit.test.ts` for a future binding test that
...
- Sparking → SVW on `a2136557` (t10 lesson-plan prompt): 3 catches +
  ...
  a phase-resolution coverage gap (F9 edge case "coverage
  primarily up to a cutoff age" had no test); `m.content.text` access in
...
- **Pre-execution self-dispatch** caught the highest-leverage defects
  ...
  _self-dispatched_, not peer-dispatched.
