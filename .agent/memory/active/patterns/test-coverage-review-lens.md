...
> **POLARITY: PATTERN.** This entry names a _shape to repeat_, not a failure
...
lens to the failing tests _first_. If most fail Q3 or Q4, the rewrite removes
...
`e2e-tests/`. Headline numbers:

| File                                      | Tests | Keep |     Demote |               Fold | Delete |                   System-transport proof retained |
| ----------------------------------------- | ----: | ---: | ---------: | -----------------: | -----: | ------------------------------------------------: |
| `get-model.e2e.test.ts`                   |    10 |    0 |          0 |                  0 |     10 | already exists in `tool-call-success.e2e.test.ts` |
...
The lens is for _review_, not for _initial test authoring_. Author tests by
TDD at the level the behaviour is being designed at. The lens runs _after_
...
- Dependency injection for testability — the DI rationale the lens leans on at Q4.
