...
all run. That sense is reliable for _self-contained tasks where
the agent's own judgement is load-bearing_. It is **unreliable as
...
that the _feel-state of completion arrives before the evidence of
completion_, and the agent then reports the feel-state as the
...
as the canonical _pattern + PDR composition_ per [PDR-014
...
_failure shape_ the rule is built to prevent: the agent's
...
the underlying shape _"the doctrine names an external loop; the
agent reports as if the loop fired."_

1. **Build-without-firing (tripwire surface).** A tripwire,
   protection, or rule is _installed_ in a session and the
   installation is reported as the evidence that the protection
   works. No firing event is observed — the artefact is the
   evidence. Diagnostic phrasing: _"installed"_, _"in place"_,
   _"now covered"_, _"protection lands"_ — said without a record
...
2. **Land-without-exercising (deferral surface, session-
   ...
   convenience phrase (_"budget consumed"_, _"out of scope"_,
   _"for later"_, _"ran out of time"_) that fails one or more of
...
3. **Compress-without-conversation (deferral surface, mid-
   ...
   momentum: the to-do shape ("in*progress → completed") wins
   against an explicit gating instruction ("owner adjudicates
   each"). Diagnostic phrasing: *"completed all to-dos"_,
   _"executed the plan"_, _"applied the disposition"\_ — said
...
opener using the phrase _"owner-paced, owner-gated per-file
disposition"_ three times, despite PDR-026 §Deferral-honesty
...
intervened mid-execution: _"failing to follow the plan is not
confusion, it's a fuck up"_; directed reset of four directive
files to HEAD; only a narrow technical micro-fix survived. The
incident is captured at length in
the experience note `2026-04-22-the-plan-was-not-the-conversation`.
...
diagnostic phrasing (_"completed the plan"_) differ from
Instance 2 (session-close, _"budget consumed"_).
...
as the evidence. The feel-state is not lying — the work _does_
...
- _"Tripwire installed"_ (no firing event named).
- _"Honestly deferred"_ (no named constraint, no evidence, no
  falsifiability).
- _"Completed all to-dos"_ (when the to-dos themselves required
  per-item owner conversation).
- _"Limit raised because the residual content is high-signal"_
  (when limit-raises are owner-only).
- _"Phase D landed"_ (when Phase D landed only the artefact
...
the agent's _genuine_ feel-state misaligning with the doctrine's
_required_ evidence loop, and trying harder produces more
...
4. **Treat the agent's own assessments as findings, not
   actions.** Per the experience entry: _"a reviewer
   recommendation is a finding, not an action."_ The same is
...
- **[PDR-026 §Deferral-honesty
  discipline](../../../practice-core/decision-records/PDR-026-per-session-landing-commitment.md#deferral-honesty-discipline)** —
  the rule-side of the canonical _pattern + PDR composition_
...
- **[PDR-014 §Graduation-target routing
  ...
  _pattern-side_ of an existing rule-side, not a new top-level
  artefact.
- **The experience note `2026-04-22-the-plan-was-not-the-conversation`** —
