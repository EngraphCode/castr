...
polarity: pattern
use_this_when: 'a plan proposes a new parallel function (fooScoped, fooV2) mirroring an existing signature — check whether the signature differs structurally before authoring it'
category: process
status: provisional
...
If the answers are _yes / no / no_, the new function is a
...
- `.agent/skills/plan/` — the plan-authoring surface that
