...
scaffolds that flow from that directive subtly encode a _misreading_
...
1. **Identify the templates and scaffolds the doctrine flows
   ...
   from these surfaces.
2. **Read each template through the _new_ doctrine, not the old
   ...
   mode the directive sharpening was _fixing_.
