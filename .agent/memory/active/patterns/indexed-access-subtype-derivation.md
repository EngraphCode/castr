...
This merges two discriminated variants into one flat type. The compiler cannot enforce the schema's constraint that a unit has _either_ `unitSlug` _or_ `unitOptions`, never both. If the schema adds a required field, `ApiUnit` silently drifts.
...
- Schema changes propagate automatically through the chain via the schema codegen step
