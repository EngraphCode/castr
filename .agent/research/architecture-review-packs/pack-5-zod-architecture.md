...
1. Severity: high
   File: [zod-parser.object.ts](../../../lib/src/schema-processing/parsers/zod/types/zod-parser.object.ts#L39)
   File: [zod-parser.object-policy.ts](../../../lib/src/schema-processing/parsers/zod/policy/zod-parser.object-policy.ts#L14)
...
2. Severity: high
   File: [zod-parser.object.ts](../../../lib/src/schema-processing/parsers/zod/types/zod-parser.object.ts#L66)
   File: [zod-parser.composition.ts](../../../lib/src/schema-processing/parsers/zod/composition/zod-parser.composition.ts#L52)
   File: [zod-parser.union.ts](../../../lib/src/schema-processing/parsers/zod/composition/zod-parser.union.ts#L50)
   File: [zod-parser.intersection.ts](../../../lib/src/schema-processing/parsers/zod/composition/zod-parser.intersection.ts#L39)
...
3. Severity: high
   File: [zod-parser.references.ts](../../../lib/src/schema-processing/parsers/zod/registry/zod-parser.references.ts#L40)
   File: [zod-parser.references.ts](../../../lib/src/schema-processing/parsers/zod/registry/zod-parser.references.ts#L54)
   File: [zod-ast.declarations.ts](../../../lib/src/schema-processing/parsers/zod/ast/zod-ast.declarations.ts#L33)
   File: [ADR-032-zod-input-strategy.md](../../../docs/architectural_decision_records/ADR-032-zod-input-strategy.md#L96)
...
4. Severity: high
   File: [zod-parser.zod4-formats.ts](../../../lib/src/schema-processing/parsers/zod/types/zod-parser.zod4-formats.ts#L21)
   File: [zod-parser.defaults.ts](../../../lib/src/schema-processing/parsers/zod/modifiers/zod-parser.defaults.ts#L57)
   File: [string-formats.zod4.ts](../../../lib/tests-fixtures/zod-parser/happy-path/string-formats.zod4.ts#L30)
   File: [primitives.ts](../../../lib/src/schema-processing/writers/zod/generators/primitives.ts#L127)
   File: [primitives.ts](../../../lib/src/schema-processing/writers/zod/generators/primitives.ts#L159)
   File: [scenario-2-zod-roundtrip.integration.test.ts](../../../lib/tests-transforms/__tests__/scenario-2-zod-roundtrip.integration.test.ts#L47)
   File: [payloads.ts](../../../lib/tests-fixtures/zod-parser/happy-path/payloads.ts#L97)
...
5. Severity: medium
   File: [scenario-2-zod-roundtrip.integration.test.ts](../../../lib/tests-transforms/__tests__/scenario-2-zod-roundtrip.integration.test.ts#L47)
   File: [scenario-4-zod-via-openapi.integration.test.ts](../../../lib/tests-transforms/__tests__/scenario-4-zod-via-openapi.integration.test.ts#L33)
   File: [scenario-6-zod-via-json-schema.integration.test.ts](../../../lib/tests-transforms/__tests__/scenario-6-zod-via-json-schema.integration.test.ts#L113)
   File: [transform-helpers.ts](../../../lib/tests-transforms/utils/transform-helpers.ts#L68)
   File: [transform-helpers.ts](../../../lib/tests-transforms/utils/transform-helpers.ts#L238)
   File: [zod-round-trip-limitations.md](../../../docs/architecture/zod-round-trip-limitations.md#L116)
   File: [metadata.ts](../../../lib/src/schema-processing/writers/zod/metadata.ts#L19)
...
- [ADR-035-transform-validation-parity.md](../../../docs/architectural_decision_records/ADR-035-transform-validation-parity.md#L62) still says non-canonical but lossless input should remain parseable, but the current Zod detection layer rejects chains such as `.email()`, `.uuid()`, and `.int()` rather than accepting them as non-canonical input; see [zod-parser.detection.ts](../../../lib/src/schema-processing/parsers/zod/zod-parser.detection.ts#L146) and [zod-parser.detection.unit.test.ts](../../../lib/src/schema-processing/parsers/zod/zod-parser.detection.unit.test.ts#L139).
- [README.md](../../../lib/src/schema-processing/writers/zod/README.md#L71) and [ADR-031-zod-output-strategy.md](../../../docs/architectural_decision_records/ADR-031-zod-output-strategy.md#L20) still read more like full metadata-preservation claims than the implemented `.meta()` subset supports; the live writer emits only a narrow key set and normalises `example` into `examples`; see [metadata.ts](../../../lib/src/schema-processing/writers/zod/metadata.ts#L19).
- [native-capability-matrix.md](../../../docs/architecture/native-capability-matrix.md#L22) still labels Zod `date-time` as a native match without noting the repo's current canonical `z.iso.datetime()` offset narrowing; the limitation is admitted in [README.md](../../../lib/src/schema-processing/writers/zod/README.md#L39) and exercised in [zod-format-functions.integration.test.ts](../../../lib/tests-transforms/__tests__/zod-format-functions.integration.test.ts#L162).
...
- [json-schema-parser.md](../../../.agent/plans/current/complete/json-schema-parser.md) remains historical remediation context; the review sweep still decides the next implementation slice, not the old queued parser-build story.
