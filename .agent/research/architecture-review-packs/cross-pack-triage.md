...
> [!NOTE]
> 5 of 8 findings resolved in [format-specific-drift-remediation.md](../../plans/current/complete/format-specific-drift-remediation.md). Remaining 3 findings (RC-4.2/4.3/4.4) addressed in RC-7: fail-fast rejection seam and doc caveats close the silent-acceptance gap. Full parser expansion remains a future capability.
...
| 2   | JSON Schema: `parseJsonSchemaDocument()` is only a `$defs` extractor                      | Pack 4.1    | ✅ (RC-7 caveat + seam) | [parsers/json-schema/index.ts](file:///Users/jim/code/personal/castr/lib/src/schema-processing/parsers/json-schema/index.ts)                                                                                                                                      |
| 3   | JSON Schema: no explicit unsupported-surface rejection seam                               | Pack 4.2    | ✅ (RC-7 fail-fast)     | [json-schema-parser.core.ts](file:///Users/jim/code/personal/castr/lib/src/schema-processing/parsers/json-schema/json-schema-parser.core.ts)                                                                                                                      |
...
**Dependency:** RC-4 items depend on honest proofs (RC-1) to verify their fixes. All 8 findings now resolved or addressed with fail-fast seam and caveats.
