...
| Portable export           | Show that exported constraints accept/reject discriminating values consistently; surface every intentional capability loss.                                                              |
...
| Maintenance observation   | Record duplicate authoring, required adapters and regeneration burden as non-gating context when an intended project integrates the tools.                                               |
...
**Plan:** pin evidence and authority → inspect current capabilities separately → reconcile with direction and counterframes → audit claims → save the comparison. Success means a reader can distinguish current implementation, ratified requirements, proposals and unknowns, and understand the responsibility and fidelity boundaries of either tool and their composition. Scope excludes migration implementation, a market survey and benchmarking. The budget is two protected evidence passes and one bounded adversarial review; closure requires disposition of material contradictions, not exhaustive repository review.

| Basis / method pass            | Question and scale                                               | Discriminating evidence                                                              | Characteristic blind spot                                              |
| ------------------------------ | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| B1 / M1: application delivery  | One team, procedure boundary, current implementation             | Existing runtime/client/validator capability and the contract boundary Castr retains | Underweighting future cross-project reuse                              |
...
- **BC2, semantic preservation → reusable contract:** faithful multi-target contracts require exact mappings and reliable generation. Usefulness is owner-set and needs no adoption proof; later maintenance observations are context, not an acceptance gate.
...
| Using both is automatically the best architecture     | Rejected: each intended project must preserve a named responsibility boundary and the semantics that cross it. |
...
**World-return contract:** Project maintainers own any subsequent integration decision; an integration implementer owns its correctness evidence. At the next actual integration, pin the selected revisions and exercise one representative operation through source → contract artefact → consuming application. Require zero unexplained acceptance or parsed-output differences in that declared fixture set and correct wire envelopes/encoding. Reopen this comparison if the official exporter preserves the demonstrated constraints or Castr’s public roots/JSON Schema surface changes. No recurring monitoring has been scheduled.
...
The new official OpenAPI exporter should change any future composition assessment of a tRPC companion. Its concrete fidelity limits explain the boundary Castr must implement. Castr establishes that distinction through demonstrated preservation and usable public interfaces, rather than a broader format list or an assumed contrast with yesterday’s tRPC.
