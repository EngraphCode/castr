      ...
      if (isObject(bundle)) {
        ...
        if (endpointsPath && fs.existsSync(endpointsPath)) {
          ...
          const endpointOps = Array.isArray(endpoints)
            ? endpoints.map((entry) =>
                isObject(entry) && typeof entry.operationId === 'string' ? entry.operationId : null,
              )
            : [];
        ...
        if (openApiPath && fs.existsSync(openApiPath)) {
          ...
          if (openApiOk) {
            const reference = sdk;
            const stableEqual = stableStringify(reference) === stableStringify(openApi);
            ...
            const outputIds = outputOps
              ...
              .sort();
            const idsOk =
              referenceIds.length === outputIds.length &&
