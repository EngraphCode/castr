/**
 *
 * EXAMPLE, not live code, imports do not exist.
 *
...
export type Normalize200<R> = 200 extends keyof R
  ? R & { 200: R[200] }
  : '200' extends keyof R
    ? Omit<R, '200'> & { 200: R['200'] }
    : never;

export type NormalizedResponsesFor<
  P extends ValidPath,
...
export function isValidPathParameter(parameterType: unknown, value: unknown): boolean {
  ...
  if (parameterType === 'keyStage') {
    const allowed: readonly string[] = KEY_STAGES;
    return allowed.includes(value);
  }
  if (parameterType === 'subject') {
    const allowed: readonly string[] = SUBJECTS;
    return allowed.includes(value);
  }
  if (parameterType === 'type') {
    const allowed: readonly string[] = ASSET_TYPES;
    return allowed.includes(value);
  }
...
export interface ValidParameterCombination<P extends ValidPath, K extends PathGroupingKeys> {
  params?: string;
  path: P;
...
export type ResponsesForPath<P extends ValidPath> = PathOperation['path'] extends P
  ? PathOperation['responses']
  : never;
export type ResponseForPathAndMethod<
  P extends ValidPath,
  M extends AllowedMethodsForPath<P>,
> = PathOperation['path'] extends P // if path extends p and method extends m, then return the responses
  ? PathOperation['method'] extends M
    ? PathOperation['responses']
    : never
  : never;
...
export const ERROR_RESPONSE_CODES = Object.keys(RESPONSE_CODES).filter(
...
export function getResponseCodesForPathAndMethod(
  path: string,
  method: string,
): ValidResponseCode[] {
  if (!isValidPath(path)) {
