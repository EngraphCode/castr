...
1. **Design gate**: Has the design intent been explored and confirmed
   ...
   has multiple valid paths, run `engraph-metacognition` first to explore
   ...
   **Verdict-vs-menu discipline** (per
   ...
   - _Unknown-to-agent design intent_ — the agent has no strong basis
   ...
   - _Agent has analysed and has a verdict_ — present the verdict
     ...
     synthesis the agent has already done. The diagnostic is: _could
     the agent rank these options by evidence already in context?_ If
...
When a plan's input is _"apply all of X"_ — every audit finding,
...
8. **Foundation alignment** — explicit references to principles.md,
   testing-strategy.md, requirements.md
