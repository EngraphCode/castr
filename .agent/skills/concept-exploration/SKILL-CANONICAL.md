...
- When options or a contested decision are already well formed, use the estate's formed-decision
  workflow (`reason`, the Decision Lenses in `principles.md`, and the `PDR-057` § Four-Lens
  dissolution test) instead of re-running this standalone pre-decision exploration.
