...
verbal forms (_undo_, _revert_, _roll back_, _throw away_, _get rid
of_, _reset_, _restore_) and operational forms (`git checkout --`,
...
4. **Render the diagnosis in chat**: the change being undone, the
   ...
   destructive operations that are explicitly _not_ recommended,
   and the reasoning.
...
- [`read-before-asking` rule](../../rules/read-before-asking.md)
  — the empirical-question reformulation; the same _render the
  diagnosis, then halt-ask_ shape, applied to questions instead of
  to undo operations.
...
- Codex and other cross-tool runtimes:
  `.agents/skills/engraph-undo-change/SKILL.md`
- Claude Code: `.claude/skills/engraph-undo-change/SKILL.md`
...
adapters resolve to this canonical file through the `engraph-` wrapper name.
