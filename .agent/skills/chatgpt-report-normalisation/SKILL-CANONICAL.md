...
citation map that is _stronger_ than the DOCX `_rels` set (which has URLs but
...
6. Render references as a deduplicated table at the end of the clean file.
   ...
   - Append a `## References` section at the bottom of the clean file with
     thematic subsections grouping refs by source family (e.g. _Sentry
     product documentation_, _PostHog pricing_, _Third-party reviews &
     community_). Each entry is a single line:
