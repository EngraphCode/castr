...
description: >-
  ...
  pair to engraph-metacognition's inward reflection. Use when facing a gnarly problem, choice, or
...
thing (`metacognition`), and not authoring a repo plan artefact (`engraph-plan`, which _uses_ these
...
a live decision, route to `engraph-concept-exploration` with its own bounded purpose; do not disguise
...
3. **Surface the warrant — and its falsifier.** For the key claim or chosen option, what unstated
   ...
   _precondition_ out loud and verify the situation's actual structure meets it before asserting
...
4. **Decide proportionately to reversibility and stakes.** One-way door or two-way? Reversible →
   ...
   verify eagerly first-hand; an expensive-to-reverse decision resting on an _inferred_ (not
