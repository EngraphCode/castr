...
description: >-
  ...
  deep — invoked naturally as wrap up safely / we are done here, at any
  boundary where the seat or session is ending. session-handoff runs inside
  wrap as its continuity component, never as an alternative close — route by
...
metaloss loop, and the exit contract, and summons the other skills for the
work. Ported 2026-08-24 from the OCE Practice repo at owner word (PR #48
review thread, "we need the wrap skill brought over from OCE"); the OCE
...
recursive exploration of the metaloss"). Conservation here is governed by
this estate's own doctrine:
...
Every session closes with wrap, ordinary or deep — that is the rule; the
cases below are emphasis, not extra conditions:
...
For a mid-session capture WITHOUT closing, run
[`session-handoff`](../session-handoff/SKILL-CANONICAL.md)'s loss-scan
discipline directly instead — the seat stays live there; wrap is for
ends. A mid-cycle retirement under budget pressure additionally follows
PDR-063's five-step protocol; wrap supplies the depth of the record it
freezes.
...
2. **Verify work safety with evidence.** WORK IS SAFE only when committed
   ...
   local commit, caught only by first-hand verification).
3. **Run [`session-handoff`](../session-handoff/SKILL-CANONICAL.md)** —
   ...
   context scan).
4. **Consolidate, conditionally.** If session-handoff's consolidation
   gate fires, summon
...
5. **Check the arc.** If this session closed a significant arc (cost,
   ...
   retrospective — routed, not auto-run (this estate has no retrospective
   skill yet; OCE's is the reference:
   ...
   If the session graduated anything, confirm each graduation carries a
   decidable prediction line per this estate's graduation discipline
   ([`new-rule-vs-pdr-clause`](../../rules/new-rule-vs-pdr-clause.md)).
6. **Run the metaloss recursion (owned here).** The loss scan is itself
   an artefact that can lose information. Scan the scan, and repeat until
   the fixed point; these are its named passes:
...
7. **Report.** The final owner-facing message: landed outcome against
   the session's landing target (PDR-026), safety evidence, what is
   conserved where, what is deliberately context-only with reasons, and
   the claims/monitors/comms disposition — with the closeout broadcasts
   session-handoff and
...
without the dead context. A wrap whose metaloss pass finds nothing on its
FIRST iteration was not entered genuinely (the founding instances each
surfaced real items: a volatile index, an uncommitted cure, a promise
without an owner); a wrap that cannot name its fixed point has not
finished.
