...
_after_ a full message has been drafted) into an **active, pre-draft tripwire**:
...
iterating on a draft message _before_ the bundle is staged — for
...
_after_ the full message has been drafted. Every failure becomes rework. This
skill installs a **pre-draft** tripwire: constraints are enumerated inline
_before_ the agent drafts, and `pnpm agent-tools:check-commit-message` runs the same
commitlint config _before_ `git commit` is invoked. The live hook is still the
...
_"all quality gates are blocking always, the orchestrator is not a quality
...
**Diagnostic discipline.** When _any_ enforcer fires, before proposing any
response, name _which_ enforcer is firing on _which_ surface and _with
what authority_. The questions to ask:
...
The orchestrator's signals are _important_. They surface the consolidation
...
- `.agents/skills/engraph-commit/SKILL.md` — cross-tool alias used by Codex,
  Cursor, Gemini, and other `.agents/` consumers.
- `.claude/skills/engraph-commit/SKILL.md` — Claude Code adapter.
...
`pnpm --filter @engraph/agent-tools skills-adapter-generate` and verify
