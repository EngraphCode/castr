...
│       │    not yet needed)     │                              │
│       └─→ (future platforms)   └─→ .codex/agents/ (Codex)    │
│                                                              │
...
**Codex** — Rules are picked up through the entrypoint chain and, when
needed, skill wrappers under `.agents/skills/` can point back to
`.agent/rules/`.
...
**Codex** — Uses `.codex/config.toml` plus `.codex/agents/*.toml` for
reviewer and domain-expert project agents. Skills remain in
`.agents/skills/` for command-shaped workflows only.
