...
| OpenAI Codex          | (via entrypoint chain) | `.codex/config.toml` + `.codex/agents/*` | `.agents/skills/*/SKILL.md` | Proven, production use |
...
### Project Agents (`.codex/config.toml` + `.codex/agents/*.toml`)

Codex reviewer and domain-expert roles are registered as project agents.
...
**Per-agent adapter**: `.codex/agents/{agent-name}.toml`
...
Keep these adapters thin. They should only carry activation metadata and a pointer back to the canonical template.

### Skills (`.agents/skills/*/SKILL.md`)

Codex skills remain the correct wrapper shape for repo-local skills and `jc-*` command workflows.
...
| -------------- | ------------------------------- | ------------------------------- | ------------------------------------- | --------------------------------------------------- |
| Rule           | `.claude/rules/{name}.md`       | `.cursor/rules/{name}.mdc`      | (via commands)                        | (via entrypoint chain / skills when command-backed) |
| Agent/Reviewer | `.claude/agents/{name}.md`      | `.cursor/agents/{name}.md`      | `.gemini/commands/review-{name}.toml` | `.codex/config.toml` + `.codex/agents/{name}.toml`  |
