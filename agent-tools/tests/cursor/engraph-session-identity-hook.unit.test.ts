...
import { planCursorSessionIdentityHook } from '../../src/cursor/engraph-session-identity-hook';
...
describe('planCursorSessionIdentityHook', () => {
  ...
  it('sets PRACTICE_AGENT_SESSION_ID_CURSOR and includes session identity context', () => {
    ...
    expect(plan.output.env).toStrictEqual({
      ...
      ENGRAPH_AGENT_IDENTITY_OVERRIDE: expectedDisplayName,
    });
    ...
    expect(plan.output.user_message).toBe(
      `Oak · ${expectedDisplayName} — suggested Composer tab title; details in .cursor/engraph-composer-session.local.json`,
    );
    expect(plan.mirror).toStrictEqual({
      absolutePath: '/repo/.cursor/engraph-composer-session.local.json',
      payload: {
        schema: 'engraph.cursor-composer-session.v1',
        updatedAt: nowIso,
  ...
  it('skips the mirror side effect when the hook environment asks for it', () => {
    const plan = planCursorSessionIdentityHook({
      ...
      environment: {
        ...
        ENGRAPH_SKIP_COMPOSER_SESSION_MIRROR: '1',
      },
      fallbackProjectDir: projectDir,
      nowIso,
    });
    ...
    expect(plan.output.env).toStrictEqual({
      ...
      ENGRAPH_AGENT_IDENTITY_OVERRIDE: deriveIdentity('unit-test-hook-seed-xyz').displayName,
    });
