...
// (PRACTICE_AGENT_SESSION_ID_CLAUDE seed + ENGRAPH_AGENT_IDENTITY_OVERRIDE name).
...
const senderWithId = deriveCollaborationIdentity({
  ...
  model: sender.model,
  env: {
    ENGRAPH_AGENT_IDENTITY_OVERRIDE: sender.agent_name,
...
const recipientWithId = deriveCollaborationIdentity({
  ...
  env: {
    ENGRAPH_AGENT_IDENTITY_OVERRIDE: recipient.agent_name,
    CODEX_THREAD_ID: recipientCodexThreadId,
  },
}).agentId;

// Recipient blocks written from --to-* flags are address relays: the written
// block omits naming_schema_version (provenance unknowable to the sender).
