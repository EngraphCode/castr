  ...
  it('renders every segment for a full linked-worktree payload', () => {
    expect(
      renderStatusline({
        ...
        dirty: false,
        worktree: 'engraph-wt-eef',
        usedPercentage: 12,
        model: 'Opus 4.7',
      }),
    ).toBe(
      `${MAGENTA}Fragrant Creeping Sapling${RESET} ${BOLD_GREEN}➜${RESET} ${CYAN}engraph-wt-eef${RESET}` +
        ` ${BOLD_BLUE}git:(${RED}feat/eef-explore-evidence${BOLD_BLUE})${RESET}` +
        ` ${BOLD_BLUE}wt:(${CYAN}engraph-wt-eef${BOLD_BLUE})${RESET} ctx:12% [Opus 4.7]`,
    );
  });
