...
```typescript
import { SERVER_INSTRUCTIONS } from '@engraph/curriculum-sdk/public/mcp-tools.js';
...
The `ENGRAPH_CONTEXT_HINT` is **dynamically generated** from `AGENT_SUPPORT_TOOL_METADATA` via `generateContextHint()` and included in `structuredContent` of every tool response, reinforcing guidance after each tool call:

```typescript
export const ENGRAPH_CONTEXT_HINT = generateContextHint();
```
...
} from '@engraph/curriculum-sdk/public/mcp-tools.js';
...
Server instructions (`SERVER_INSTRUCTIONS`) and context hints (`ENGRAPH_CONTEXT_HINT`) are **dynamically generated** from `AGENT_SUPPORT_TOOL_METADATA`. Adding a tool to the metadata with `callAtStart: true` automatically includes it in instructions. No manual updates needed.
...
- [Schema-first execution doctrine (castr keeps it in requirements.md)](../../.agent/directives/requirements.md)
