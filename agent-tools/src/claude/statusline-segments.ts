...
export interface Segments {
  ...
  /** The Claude.ai rate-limit gauges (`5h:…%(…) · w:…%(…)`); absent off Claude.ai or before the first response. */
...
/**
 * The Claude.ai rate-limit gauges — the five-hour window (`5h`) and week (`w`,
 * seven-day) consumed percentages, each with a parenthesised countdown to its
 * reset, e.g. `5h:33%(2h) · w:55%(3d)`. The percentage carries the consumption
...
function formatRateLimits(parts: StatuslineParts): string | undefined {
  const joined = joinPresent([
    rateLimitGauge('5h', parts.fiveHourPercentage, parts.fiveHourResetSeconds),
