...
function createValidTurboSummaryJson() {
  return {
    ...
    tasks: [
      {
        taskId: '@engraph/pkg-a#build',
        task: 'build',
        package: '@engraph/pkg-a',
        execution: {
          exitCode: 0,
        },
        cache: {
      ...
      {
        taskId: '@engraph/pkg-b#test',
        task: 'test',
        package: '@engraph/pkg-b',
        execution: {
          exitCode: 1,
        },
        cache: {
...
describe('parseTurboSummary', () => {
  ...
  it('accepts tasks with a null log file', () => {
    const result = parseTurboSummary({
      ...
      tasks: [
        {
          taskId: '@engraph/pkg-a#smoke:dev:stub',
          task: 'smoke:dev:stub',
          package: '@engraph/pkg-a',
          execution: {
            exitCode: 0,
...
describe('formatSummaryTable', () => {
  ...
  it('formats pass and fail tasks with cache status and totals', () => {
    ...
    expect(table).toContain('| @engraph/pkg-a#build | PASS | HIT | 1.2s |');
    expect(table).toContain('| @engraph/pkg-b#test | FAIL | MISS | 0ms |');
    expect(table).toContain(
...
describe('formatAnnotations', () => {
  it('formats GitHub error annotations only for failed tasks', () => {
    ...
    expect(annotations).toBe(
      '::error file=packages/pkg-b/.turbo/turbo-test.log::@engraph/pkg-b#test failed',
    );
  });
  ...
  it('omits the file property when a failed task has no log file', () => {
    ...
    expect(annotations).toBe('::error::@engraph/pkg-a#smoke:dev:stub failed');
  });
});
