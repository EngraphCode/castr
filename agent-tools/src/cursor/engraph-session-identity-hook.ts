...
const COMPOSER_MIRROR_FILE = 'engraph-composer-session.local.json';
const MIRROR_SCHEMA = 'engraph.cursor-composer-session.v1';
...
export interface CursorSessionIdentityHookEnvironment {
  ...
  readonly ENGRAPH_SKIP_COMPOSER_SESSION_MIRROR?: string;
...
export function planCursorSessionIdentityHook(
  ...
  const override = input.environment.ENGRAPH_AGENT_IDENTITY_OVERRIDE?.trim();
...
function shouldSkipMirror(environment: CursorSessionIdentityHookEnvironment): boolean {
  return environment.ENGRAPH_SKIP_COMPOSER_SESSION_MIRROR?.trim() === '1';
}
