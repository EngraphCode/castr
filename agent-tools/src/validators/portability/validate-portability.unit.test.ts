...
describe('getSkillPermissionIssues', () => {
  it('reports a missing Skill() permission when a Claude command adapter exists without a settings entry', () => {
    expect(
      getSkillPermissionIssues({
        claudeCommandFiles: ['.claude/commands/engraph-start-right-quick.md'],
        claudeSettingsPermissions: ['Skill(engraph-plan)', 'Skill(engraph-plan:*)'],
      }),
    ).toContainEqual(expect.stringContaining('engraph-start-right-quick'));
  });
    ...
    expect(
      getSkillPermissionIssues({
        ...
        claudeSettingsPermissions: [
          'Skill(engraph-start-right-quick)',
          'Skill(engraph-start-right-quick:*)',
        ],
      }),
    ).toStrictEqual([]);
  ...
  it('does not require the wildcard variant — only the base Skill() entry', () => {
    expect(
      getSkillPermissionIssues({
        claudeCommandFiles: ['.claude/commands/engraph-gates.md'],
        claudeSettingsPermissions: ['Skill(engraph-gates)'],
      }),
    ).toStrictEqual([]);
  });
  ...
  it('ignores non-Skill permissions in the allow list', () => {
    ...
    ).toContainEqual(expect.stringContaining('engraph-gates'));
  });
});
