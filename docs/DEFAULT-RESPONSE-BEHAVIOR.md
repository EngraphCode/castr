...
This is **not an error** in your API or in `openapi-zod-client`. It's an informational warning about how the tool handles OpenAPI endpoints that only define a `default` response status code.
...
The OpenAPI spec recommends **defining explicit status codes** instead of relying solely on `default`. This is why `openapi-zod-client` issues a warning and, by default, **ignores** endpoints with only `default` responses.

---

## The Solution: `defaultStatusBehavior`

The `defaultStatusBehavior` option controls how `openapi-zod-client` handles endpoints with only `default` status codes.
...
1. `openapi-zod-client` correctly detects endpoints with only `default` responses
...
```bash
npx openapi-zod-client ./openapi.yaml -o ./generated.ts
...
```bash
npx openapi-zod-client ./openapi.yaml -o ./generated.ts --default-status-behavior auto-correct
