...
This guide provides comprehensive instructions for integrating `@engraph/castr` with the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/), enabling AI models to interact with OpenAPI-defined APIs through structured tool definitions.
...
The Model Context Protocol (MCP) provides a standardized way for AI models to discover and invoke external tools. `@engraph/castr` automatically converts OpenAPI operations into MCP-compatible tool definitions with:
...
@engraph/castr ./petstore.yaml -o ./src/api-client.ts --emit-mcp-manifest ./mcp-tools.json

# Manifest only (no client generation)
@engraph/castr ./petstore.yaml --emit-mcp-manifest ./mcp-tools.json
```
...
```typescript
import { z } from 'zod';
import { isMcpToolInput, formatMcpValidationError } from '@engraph/castr';
...
`@engraph/castr` extracts authentication requirements from OpenAPI security definitions and associates them with each operation.
...
```typescript
import { logger } from '@engraph/castr';
