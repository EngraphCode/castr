# Migration Guide: Legacy to @engraph/castr

This guide helps you migrate to `@engraph/castr` from legacy forks or the original upstream library.
...
`@engraph/castr` is a maintained fork of `@engraph/castr` with significant improvements:
...
- ✅ **Caster Model architecture** - Canonical internal representation for all formats
...
```bash
npx @engraph/castr ./openapi.yaml -o ./api-client.ts
```
...
```typescript
import { generateZodClientFromOpenAPI, getZodiosEndpointDefinitionList } from '@engraph/castr';
...
- [ ] Update package name: `@engraph/castr` → `openapi-zod-validation`
