...
The original `openapi-zod-client` library generated:
...
1. **openapi-zod-client**: Generates Zod schemas + Zodios client (opinionated, axios-based)
...
**Should @oaknational/openapi-to-tooling:**

- **Option A**: Generate complete SDKs with HTTP client (like original openapi-zod-client)?
- **Option B**: Generate building blocks, let consumers choose HTTP client?

### Key Insight from Research

After researching both openapi-zod-client and openapi-fetch:

- **openapi-zod-client** = Schema generation + Zodios HTTP client (coupled)
- **openapi-fetch** = Type-safe HTTP client only (no schema generation)

**Our library's role:**

- Replace the **schema generation and validation** parts of openapi-zod-client (non-HTTP parts)
- Enable a **separate SDK workspace** to replace openapi-fetch's role (HTTP client integration)
...
| **@oaknational/openapi-to-tooling** | Schemas + metadata + validation | **Building blocks** |
| **openapi-fetch** | HTTP only | Building block |
| **openapi-zod-client** | All-in-one | **Monolithic** |
...
```typescript
// @oaknational/openapi-to-tooling provides:
import { endpoints, schemas, validateRequest, validateResponse } from '@oaknational/openapi-to-tooling';
...
**Replaces parts of openapi-zod-client:**
...
- openapi-zod-client: All-in-one approach (schemas + Zodios HTTP client)
