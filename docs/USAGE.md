...
Complete guide to using `openapi-zod-client` for generating type-safe API clients with runtime validation.
...
# or
pnpm add openapi-zod-client
# or
yarn add openapi-zod-client
```
...
`openapi-zod-client` offers three templates for different use cases:
...
```bash
npx openapi-zod-client ./openapi.yaml -o ./src/schemas.ts --template schemas-only
```
...
# or explicitly:
npx openapi-zod-client ./openapi.yaml -o ./src/api.ts --template schemas-with-metadata
```
...
npx openapi-zod-client ./users-api.yaml -o ./src/users-client.ts --template schemas-with-client

# Generate client for orders service
npx openapi-zod-client ./orders-api.yaml -o ./src/orders-client.ts --template schemas-with-client
```
