...
test("invalid-pattern-regex", () => {
    ...
    expect(
        getZodSchema({ schema: invalidSchema }).toString() + getZodChain({ schema: invalidSchema }).toString()
    ).toMatchInlineSnapshot('"z.string().regex(/[0-9]+/).optional()"');
    expect(
        getZodSchema({ schema: controlCharacters }).toString() + getZodChain({ schema: controlCharacters }).toString()
