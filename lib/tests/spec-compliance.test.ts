...
describe("spec-compliance", () => {
    describe("MediaType schema property", () => {
        test("$ref at wrong level (mediaTypeObject instead of schema property) should throw", async () => {
                            ...
                            parameters: [
                                {
                                    ...
                                    content: {
                                        ...
                                        "*/*": { $ref: "#/components/schemas/Pet" } as any,
                                    },
                                },
                            ],
                            responses: {
            ...
            await expect(generateZodClientFromOpenAPI({ disableWriteToFile: true, openApiDoc })).rejects.toThrow(
        ...
        test("valid $ref inside schema property should succeed", async () => {
            ...
            await expect(generateZodClientFromOpenAPI({ disableWriteToFile: true, openApiDoc })).resolves.not.toThrow();
        });
    });
