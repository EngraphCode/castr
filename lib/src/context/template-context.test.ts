...
describe('getZodClientTemplateContext - MCP tools', () => {
  test('should include MCP tool metadata for operations', () => {
    const openApiDoc: OpenAPIObject = {
      openapi: '3.1.0',
    ...
    const getTool = result.mcpTools?.find((entry) => entry.method === 'get');
    ...
    const postTool = result.mcpTools?.find((entry) => entry.method === 'post');
