...
export function generateOpenAPI(ir: IRDocument): OpenAPIObject {
  return {
...
function generatePaths(operations: IROperation[]): PathsObject {
...
function generateOperation(op: IROperation): OperationObject {
...
function assignOperationMetadata(op: IROperation, operation: OperationObject): void {
  if (op.summary) {
...
function generateParameter(param: IRParameter): ParameterObject {
...
function generateResponse(response: IRResponse): ResponseObject {
...
function generateSchema(irSchema: IRSchema): SchemaObject {
  if (irSchema.$ref) {
...
function assignSchemaProperties(irSchema: IRSchema, schema: SchemaObject): void {
...
function assignSchemaComposition(irSchema: IRSchema, schema: SchemaObject): void {
  if (irSchema.allOf) {
...
function assignSchemaItems(irSchema: IRSchema, schema: SchemaObject): void {
  if (irSchema.items) {
