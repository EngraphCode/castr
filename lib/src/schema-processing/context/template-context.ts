...
export interface TemplateContextOptions {
  ...
  /**
   * When true, the writer exports all component schemas regardless of whether
   * they are referenced by endpoints. Affects writer export visibility only —
   * does not change IR construction or template-context building.
   */
...
export const getTemplateContext = (
  ...
  // Schemas-only template: skip endpoint/MCP construction entirely
  ...
  const enrichedComponents = [...irDocument.components];
  const inlineComponents = extractInlineSchemas(irDocument);
  enrichedComponents.push(...inlineComponents);
  ...
  // They are extracted from operations and depend on existing components,
  // so appending at the end preserves topological order.
  ...
  const emptyGrouping: {
    endpointsGroups: Record<string, MinimalTemplateContext>;
    commonSchemaNames: undefined;
  } = {
    endpointsGroups: {},
    commonSchemaNames: undefined,
  };

  const { endpointsGroups, commonSchemaNames } = isSchemasOnly
    ? emptyGrouping
    : processEndpointGroupingAndCommonSchemas(
        endpoints,
  ...
  const result: TemplateContext = {
    ...
    endpointsGroups,
    mcpTools,
    _ir: { ...irDocument, components: enrichedComponents },
  };
